...
test("mask function works correctly", () => {
	...
	assert.equal(
		mask("1234567890", { keepStart: 2, keepEnd: 2 }),
		"12******90",
		"keeps start and end",
	);
	assert.equal(
		mask("short", { keepStart: 4, keepEnd: 4 }),
		"short",
...
test("sanitizeDestination masks all sensitive fields", () => {
	...
	assert.ok(
		out.accountMasked.endsWith("3456"),
		"Account number should be masked",
	);
	assert.ok(
		out.beneficiaryMasked.startsWith("J"),
		"Beneficiary should be masked",
	);
});
...
test("normalizePayoutRoute handles various inputs", () => {
	...
	assert.equal(
		normalizePayoutRoute("PAYPAL"),
		"paypal_payouts_api",
		"case-insensitivity",
	);
	assert.equal(
		normalizePayoutRoute(null, "default"),
		"default",
		"fallback to default",
	);
	assert.equal(
		normalizePayoutRoute(""),
		"bank_wire",
...
test("destination JSON can be parsed when quotes are backslash-escaped", () => {
	const raw =
		'{\\"bank\\":\\"ExampleBank\\",\\"swift\\":\\"EXAMPLEX1\\",\\"account\\":\\"1234567890123456\\",\\"beneficiary\\":\\"Jane Doe\\"}';
...
test("destination JSON can be parsed when quotes are backslash-escaped", () => {
  ...
  const parsed = JSON.parse(raw.replace(/\\"/g, '"'));
	const out = sanitizeDestination(parsed);
	assert.notEqual(out.bank, "ExampleBank", "Bank name should be masked");
	assert.notEqual(out.swift, "EXAMPLEX1", "SWIFT code should be masked");
	assert.equal(out.accountMasked.endsWith("3456"), true, "Account should end with last 4 digits");
});
...
test("PPP2 route falls back to bank wire for Morocco without approval", () => {
	...
	assert.equal(
		plan.reasons.some((r) => r.toLowerCase().includes("morocco")),
		true,
	);
});
