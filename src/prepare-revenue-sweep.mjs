...
function normalizePayoutRoute(value, fallback = "bank_wire") {
	...
	const aliases = {
		...
		googlepay: "googlepay_transfer",
		google_pay: "googlepay_transfer",
		googlepay_transfer: "googlepay_transfer",
		plaid: "plaid_transfer",
		plaid_transfer: "plaid_transfer",
		crypto: "crypto_transfer",
		cryptocurrency: "crypto_transfer",
...
function buildPayoutPlan({
	...
	if (selected === "paypal_payouts_api") {
		if (countryCode === "MA") {
			...
			if (!(ppp2Approved && ppp2EnableSend)) {
				selected = "bank_wire";
				reasons.push(
					"PPP2 send not explicitly approved/enabled; falling back to bank wire",
				);
			}
	...
	if (selected === "bank_wire" && !hasBankDetails) {
		reasons.push(
			"Missing bank details; bank wire requires account/RIB/IBAN + SWIFT",
		);
	}
...
function isRevenueVoidWriteEnabled() {
	return (
		(process.env.BASE44_ENABLE_REVENUE_VOID_WRITE ?? "false").toLowerCase() ===
		"true"
	);
}
...
function isSimulationMissionRow(row) {
	...
	return (
		isSample || type.includes("simulation") || title.includes("simulation")
	);
}
...
async function revalidateMissionCsvFile(csvPath) {
	...
	if (isUnsafePath(csvPath))
		throw new Error("LIVE MODE NOT GUARANTEED (unsafe mission csv path)");
	const stream = fs.createReadStream(csvPath, { encoding: "utf8" });
	const rl = readline.createInterface({ input: stream, crlfDelay: Infinity });
	const maxAgeDays =
		Number(process.env.SWARM_MAX_MISSION_AGE_DAYS ?? "365") || 365;
	const nowMs = Date.now();
	const maxFutureSkewMs =
		Number(process.env.SWARM_MAX_FUTURE_SKEW_MS ?? "300000") || 300000;
	const minAllowedMs =
		maxAgeDays > 0 ? nowMs - maxAgeDays * 24 * 60 * 60 * 1000 : null;
	let header = null;
	let total = 0;
	for await (const line of rl) {
		...
		if (effectiveMs == null) {
			throw new Error(
				`LIVE MODE NOT GUARANTEED (missing mission timestamp: id=${String(row?.id ?? "")})`,
			);
		}
		if (effectiveMs > nowMs + maxFutureSkewMs) {
			throw new Error(
				`LIVE MODE NOT GUARANTEED (future mission timestamp: id=${String(row?.id ?? "")})`,
			);
		}
		if (minAllowedMs != null && effectiveMs < minAllowedMs) {
			throw new Error(
				`LIVE MODE NOT GUARANTEED (stale mission timestamp: id=${String(row?.id ?? "")})`,
			);
		}
		if (createdMs != null && updatedMs != null && updatedMs < createdMs) {
			throw new Error(
				`LIVE MODE NOT GUARANTEED (non-monotonic mission timestamps: id=${String(row?.id ?? "")})`,
			);
		}
	}
...
function resolveMissionFilters(args) {
	const onlyMissionIds = parseCommaList(
		args["only-mission-ids"] ??
			args.onlyMissionIds ??
			process.env.SWARM_ONLY_MISSION_IDS ??
			process.env.ONLY_MISSION_IDS ??
			"",
	);
	...
	return {
		...
		onlyMissionTitles:
			onlyMissionTitles.length > 0 ? new Set(onlyMissionTitles) : null,
	};
}
...
function missionMatchesFilters(m, filters) {
	...
	if (titleSet && titleSet.has(normalizeMissionTitle(m?.title ?? "")))
		return true;
	return false;
}

async function aggregateFromMissionsCsv(
	csvPath,
	{ currency, allowNonPositiveAmounts, enforceLiveMissions },
) {
	const stream = fs.createReadStream(csvPath, { encoding: "utf8" });
	const rl = readline.createInterface({ input: stream, crlfDelay: Infinity });
...
async function listAll(
	entity,
	{ sort = "-created_date", pageSize = 250, fields = null } = {},
) {
	const all = [];
	let offset = 0;
	while (true) {
...
async function purgeSimulationRevenueEvents(
	base44,
	{ csvPaths, dryRun, filters },
) {
	if (!Array.isArray(csvPaths) || csvPaths.length === 0)
		throw new Error("No mission CSVs provided");
	if (!dryRun && !isRevenueVoidWriteEnabled()) {
		throw new Error(
			"Refusing to purge revenue events without BASE44_ENABLE_REVENUE_VOID_WRITE=true",
		);
	}
	...
	if (simMissionIds.size === 0) {
		return {
			simMissionIds: 0,
			matchedRevenueEvents: 0,
			updatedRevenueEvents: 0,
		};
	}

	if (!base44 && !dryRun) {
		throw new Error(
			"Base44 client not configured (missing BASE44_APP_ID / BASE44_SERVICE_TOKEN)",
		);
	}
	...
	const revenueEntity =
		base44?.asServiceRole?.entities?.[revenueCfg.entityName];
	if (!revenueEntity && !dryRun)
		throw new Error(
			`Missing Base44 RevenueEvent entity (${revenueCfg.entityName})`,
		);

	const fields = [
		"id",
		revenueCfg.fieldMap.missionId,
		revenueCfg.fieldMap.status,
	].filter(Boolean);

	const canQuery = !!base44 && !!revenueEntity;
	const all = canQuery
		? await listAll(revenueEntity, { fields, pageSize: 250 })
		: [];
	const matched = canQuery
		? all.filter((r) => {
				const mid = revenueCfg.fieldMap.missionId
					? r?.[revenueCfg.fieldMap.missionId]
					: null;
				if (!mid) return false;
	...
	if (dryRun) {
		return {
			simMissionIds: simMissionIds.size,
			matchedRevenueEvents: matched.length,
			updatedRevenueEvents: 0,
		};
	}
	...
	return {
		simMissionIds: simMissionIds.size,
		matchedRevenueEvents: matched.length,
		updatedRevenueEvents: updates.length,
	};
}
...
async function migrateMissionsToLive(base44, { csvPaths, dryRun, filters }) {
	...
	const selected = filters
		? deduped.filter((m) => missionMatchesFilters(m, filters))
		: deduped;
	const entity = base44?.asServiceRole?.entities?.Mission;
	if (!dryRun && !entity)
		throw new Error("Missing Base44 client or Mission entity");
	...
	const wouldUpdateCount = results.filter(
		(r) => r?.needsUpdate === true,
	).length;
...
async function createPayoutRequest(base44, cfg, payload, { dryRun }) {
	...
	if (dryRun) {
		process.stdout.write(
			`${JSON.stringify({ entity: cfg.payoutEntityName, data })}\n`,
		);
		return { dryRun: true };
	}
	...
	const enable =
		(process.env.BASE44_ENABLE_PAYOUT_REQUESTS ?? "false").toLowerCase() ===
		"true";
	if (!enable) {
		throw new Error(
			"Refusing to create payout requests without BASE44_ENABLE_PAYOUT_REQUESTS=true",
		);
	}
	...
	try {
		...
		const created = await offlineEntity.create(data);
		return { ...created, offline: true };
	}
}
...
async function main() {
	...
	const migrateMissionsLive =
		args["migrate-missions-live"] === true ||
		args.migrateMissionsLive === true ||
		args.migrateMissions === true;
	const purgeSimulationData =
		args["purge-simulation-data"] === true ||
		args.purgeSimulationData === true ||
		args["purge-simulation-revenue"] === true;
	const revalidateAllMissions =
		args["revalidate-all-missions"] === true ||
		args.revalidateAllMissions === true;
	const csvPath =
		args.csv ??
		process.env.BASE44_MISSIONS_CSV ??
		process.env.MISSIONS_CSV ??
		null;
	const archiveDir = args["archive-dir"] ?? args.archiveDir ?? null;
	if (!migrateMissionsLive && !csvPath) {
		throw new Error(
			"Missing required: --csv <MISSIONS_CSV_PATH> (or BASE44_MISSIONS_CSV/MISSIONS_CSV)",
		);
	}

	const allowNonPositiveAmounts =
		(process.env.BASE44_ALLOW_NON_POSITIVE_REVENUE ?? "false").toLowerCase() ===
		"true";
	...
	const dest = {
		...
		...(args["dest-beneficiary"]
			? { beneficiary: args["dest-beneficiary"] }
			: {}),
	};
	...
	const countryCode = normalizeCountryCode(
		args["payout-country"] ??
			args.country ??
			process.env.BASE44_PAYOUT_COUNTRY ??
			process.env.PAYOUT_COUNTRY,
		"",
	);
	...
	if (revalidateAllMissions && csvPath) {
		...
		process.stdout.write(
			`${JSON.stringify({ ok: true, revalidatedMissions: validated })}\n`,
		);
	}
	...
	if (migrateMissionsLive || purgeSimulationData) {
		...
		if (paths.length === 0) {
			throw new Error(
				"Missing required: --archive-dir <DIR_WITH_Mission_export*.csv> and/or --csv <MISSIONS_CSV_PATH>",
			);
		}
		if (!dryRun && !base44) {
			throw new Error(
		...
		const migrated = migrateMissionsLive
			? await migrateMissionsToLive(base44, {
					csvPaths: paths,
					dryRun,
					filters,
				})
			: null;
		const purged = purgeSimulationData
			? await purgeSimulationRevenueEvents(base44, {
					csvPaths: paths,
					dryRun,
					filters,
				})
	...
	const enforceLiveMissions =
		args["allow-simulations"] === true || args.allowSimulations === true
			? false
			: (process.env.SWARM_ENFORCE_LIVE_MISSIONS ?? "true").toLowerCase() ===
				"true";

	const agg = await aggregateFromMissionsCsv(csvPath, {
		currency,
		allowNonPositiveAmounts,
		enforceLiveMissions,
	});
