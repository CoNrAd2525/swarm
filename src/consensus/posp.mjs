...
export function calculatePosp({
	...
	const revenueTotal = receipts.reduce(
		(s, r) => s + Number(r?.amount_total_usd || 0),
		0,
	);
	const uniquePayers = new Set(
		receipts
			.map((r) =>
				String(r?.payer?.email || "")
					.trim()
					.toLowerCase(),
			)
			.filter((e) => !!e),
	...
	const score = Math.round(
		txCount + revenueTotal / 100 + uniquePayers.size * 2,
	);
...
export function enforcePosp(proof, { minScore = 5, minTx = 1 } = {}) {
	const ok =
		(proof?.score || 0) >= minScore && (proof?.basis?.tx_count || 0) >= minTx;
	return {
		ok,
		minScore,
		minTx,
		score: proof?.score || 0,
		txCount: proof?.basis?.tx_count || 0,
	};
}
...
export function writePospProof(proof, { dir = "exports/posp-proofs" } = {}) {
	...
	const filePath = path.join(
		outDir,
		`posp_proof_${proof.agent_id}_${Date.now()}.json`,
	);
