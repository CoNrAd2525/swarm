...
function randomToken(len = 48) {
	return crypto
		.randomBytes(Math.max(16, Math.floor(len / 2)))
		.toString("hex")
		.slice(0, len);
}
...
function buildMatchers(values) {
	...
	for (const v of values) {
		...
		if (s.length >= 16) {
			...
			m.push({
				re: new RegExp(head.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"),
				mask: "****",
			});
			m.push({
				re: new RegExp(tail.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"),
				mask: "****",
			});
		}
	}
	...
	for (const n of names) {
		...
		console[n] = (...args) => {
			const safe = args.map((a) =>
				typeof a === "string" ? maskText(a, matchers) : a,
			);
			return orig(...safe);
		};
	}
...
function patchFetch(honey) {
	...
	globalThis.fetch = async (input, init = {}) => {
		try {
			...
			if (
				vals.includes(honey.value) ||
				vals.includes(honey.decoyBinanceKey) ||
				vals.includes(honey.decoyBinanceSecret)
			) {
				threatMonitor.reportError(
					"exfil_attempt",
					new Error("Honeytoken triggered"),
				);
			}
		} catch {}
...
function enableHoneytokens() {
	...
	if (!process.env.BINANCE_API_KEY_DECOY)
		process.env.BINANCE_API_KEY_DECOY = honey.decoyBinanceKey;
	if (
		!process.env.BINANCE_API_SECRET_DECOY &&
		!process.env.BINANCE_SECRET_KEY_DECOY
	) {
