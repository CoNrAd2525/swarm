...
function requireLiveMode(reason) {
	if (!getEnvBool("SWARM_LIVE", false)) {
		throw new Error(
			`Refusing live operation without SWARM_LIVE=true (${reason})`,
		);
	}
	if (
		...
		throw new Error(
			`LIVE MODE NOT GUARANTEED (offline mode enabled: ${reason})`,
		);
	}
	...
	if (paypalMode === "sandbox" || paypalBase.includes("sandbox.paypal.com")) {
		throw new Error(
			`LIVE MODE NOT GUARANTEED (PayPal sandbox configured: ${reason})`,
		);
	}
}
...
function shouldCreatePayPalOrders() {
	return (
		(process.env.PAYPAL_ENABLE_ORDER_CREATE ?? "false").toLowerCase() === "true"
	);
}
...
function shouldWriteToBase44() {
	return (
		(
			process.env.BASE44_ENABLE_PAYPAL_WEBHOOK_WRITE ?? "false"
		).toLowerCase() === "true"
	);
}
...
function shouldWriteRevenueFromPayPal() {
	return (
		(process.env.BASE44_ENABLE_REVENUE_FROM_PAYPAL ?? "false").toLowerCase() ===
		"true"
	);
}
...
function shouldAlertOnWebhookErrors() {
	return (
		(
			process.env.BASE44_ENABLE_ALERTS_ON_WEBHOOK_ERRORS ?? "false"
		).toLowerCase() === "true"
	);
}
...
function shouldAlertOnPayoutFailures() {
	return (
		(
			process.env.BASE44_ENABLE_ALERTS_ON_PAYOUT_FAILURES ?? "false"
		).toLowerCase() === "true"
	);
}
...
function shouldWriteMetricsToBase44() {
	return (
		(process.env.BASE44_ENABLE_PAYPAL_METRICS ?? "false").toLowerCase() ===
		"true"
	);
}
...
function validateConfig() {
	...
	if (wantWritePayoutStatus && !shouldWritePayoutLedger()) {
		throw new Error(
			"Refusing payout status writes without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
		);
	}
...
function getConfigCheck() {
	...
	const wantWriteMetrics = shouldWriteMetricsToBase44();
	const wantAlerts =
		shouldAlertOnWebhookErrors() || shouldAlertOnPayoutFailures();
	const anyWrites =
		wantWriteEvent ||
		wantWriteRevenue ||
		wantWriteMetrics ||
	...
	if (
		...
		missing.push("SWARM_LIVE");
	if (
		(anyWrites || wantAlerts) &&
		(!have("BASE44_APP_ID") || !have("BASE44_SERVICE_TOKEN"))
	)
		missing.push("BASE44_APP_ID/BASE44_SERVICE_TOKEN");
	if (wantWritePayoutStatus && !shouldWritePayoutLedger())
		missing.push("BASE44_ENABLE_PAYOUT_LEDGER_WRITE");
...
async function listAll(
	entity,
	{ fields = null, pageSize = 250, order = "-created_date", max = 2500 } = {},
) {
	const out = [];
...
function mapPayPalWebhookToPayoutItemUpdate(evt) {
	...
	const paypalBatchId =
		resource?.payout_batch_id ?? resource?.payout_item?.payout_batch_id ?? null;
	const paypalTxnId =
		resource?.transaction_id ?? resource?.payout_item?.transaction_id ?? null;
	const paypalStatus =
		resource?.transaction_status ??
		resource?.payout_item?.transaction_status ??
		null;
	const processedAt =
		resource?.time_processed ??
		resource?.time_completed ??
		evt?.create_time ??
		new Date().toISOString();

	let status = null;
	if (upper.includes("SUCCEEDED") || upper.includes("SUCCESS"))
		status = "success";
	...
	const errorMessage = resource?.errors
		? truncate(JSON.stringify(resource.errors), 500)
		: null;
...
function mapPayPalWebhookToPayoutBatchUpdate(evt) {
	...
	const batchStatus =
		resource?.batch_header?.batch_status ?? resource?.batch_status ?? null;
	...
	if (upper.includes("SUCCESS") || upper.includes("SUCCEEDED"))
		...
		ledgerStatus = "failed";
	else if (upper.includes("PROCESSING") || upper.includes("PENDING"))
...
async function applyPayoutBatchUpdate(base44, update) {
	...
	const batch = await findPayoutBatchByPayPalBatchId(
		base44,
		update.paypalBatchId,
	);
	if (!batch?.id)
		return { ok: false, updated: false, reason: "payout_batch_not_found" };
	...
	const mergedNotes =
		...
		!Array.isArray(existingNotes)
			? {
					...
					...(update.paypalBatchStatus != null
						? { paypal_batch_status: update.paypalBatchStatus }
						: {}),
	...
	const patch = payoutBatchCfg.fieldMap.notes
	...
	if (
		update.ledgerStatus === "completed" &&
		payoutBatchCfg.fieldMap.completedAt
	) {
		patch[payoutBatchCfg.fieldMap.completedAt] =
...
async function applyPayoutItemUpdate(base44, update) {
	...
	const item = await findOneBy(itemEntity, {
		[payoutItemCfg.fieldMap.paypalItemId]: update.paypalItemId,
	});
	if (!item?.id)
		return { ok: false, updated: false, reason: "payout_item_not_found" };
			...
			try {
				await txEntity.create({
					...
					[txCfg.fieldMap.description]:
						`Invalid payout item state transition ${String(current)} -> ${String(update.status)} for PayPal item ${String(
							update.paypalItemId,
						)}`,
					...
					...(txCfg.fieldMap.payoutItemId && payoutItemCfg.fieldMap.itemId
						? {
								[txCfg.fieldMap.payoutItemId]:
									item?.[payoutItemCfg.fieldMap.itemId] ?? null,
							}
						: {}),
				});
			} catch {}
	...
	const patch = {
		...(payoutItemCfg.fieldMap.processedAt
			? { [payoutItemCfg.fieldMap.processedAt]: update.processedAt }
			: {}),
		...(payoutItemCfg.fieldMap.paypalStatus
			? { [payoutItemCfg.fieldMap.paypalStatus]: update.paypalStatus }
			: {}),
	...
	if (update.status && payoutItemCfg.fieldMap.status)
		patch[payoutItemCfg.fieldMap.status] = update.status;

	const updatedItem = await itemEntity.update(item.id, patch);

	const revenueEventId = payoutItemCfg.fieldMap.revenueEventId
		? item?.[payoutItemCfg.fieldMap.revenueEventId]
		: null;
	const batchId = payoutItemCfg.fieldMap.batchId
		? item?.[payoutItemCfg.fieldMap.batchId]
		: null;

	if (batchId && payoutBatchCfg.fieldMap.notes) {
		const batch = await findOneBy(batchEntity, {
		...
		if (batch?.id) {
			...
			const mergedNotes =
				...
				!Array.isArray(existingNotes)
					? {
							...existingNotes,
							...(update.paypalBatchId &&
							!existingNotes?.paypal_payout_batch_id &&
							!existingNotes?.paypalPayoutBatchId
								? { paypal_payout_batch_id: update.paypalBatchId }
								: {}),
							paypal_synced_at: new Date().toISOString(),
							paypal_webhook_last_event_id: update.webhookEventId ?? null,
			...
			await batchEntity
				.update(batch.id, { [payoutBatchCfg.fieldMap.notes]: mergedNotes })
				.catch(() => null);
		}
	}
	...
	if (
		...
		if (revenueCfg.fieldMap.payoutBatchId && batchId)
			revPatch[revenueCfg.fieldMap.payoutBatchId] = batchId;
		...
		if (!txDeduped) {
			const amount = Number(
				payoutItemCfg.fieldMap.amount
					? item?.[payoutItemCfg.fieldMap.amount]
					: 0,
			);
			const created = await txEntity.create({
				[txCfg.fieldMap.transactionType]: "withdrawal",
				[txCfg.fieldMap.amount]: Number.isFinite(amount)
					? Number(amount.toFixed(2))
					: null,
				[txCfg.fieldMap.description]:
					`Payout to ${String(item?.[payoutItemCfg.fieldMap.recipient] ?? "")} for ${String(revenueEventId ?? "")}`,
	...
	if (batchId) {
		...
		const allFinal =
			Array.isArray(items) && items.length > 0
				? items.every((it) =>
						isFinalItemStatus(it?.[payoutItemCfg.fieldMap.status] ?? null),
					)
				: false;
		const allSuccess =
			Array.isArray(items) && items.length > 0
				? items.every(
						(it) => (it?.[payoutItemCfg.fieldMap.status] ?? null) === "success",
					)
				: false;
				...
				if (!providerId) {
					...
					if (payoutBatchCfg.fieldMap.notes) {
						...
						const ledgerStatus = payoutBatchCfg.fieldMap.status
							? batch?.[payoutBatchCfg.fieldMap.status]
							: null;
						const mergedNotes = {
							...existingNotes,
					...
					if (batchCompleted && revenueEventId && revenueCfg.fieldMap.status) {
						try {
							await revenueEntity.update(revenueEventId, {
								[revenueCfg.fieldMap.status]: "confirmed",
							});
							revenueUpdated = true;
						} catch {}
					}
...
async function writeEventToBase44Idempotent(base44, cfg, evt) {
	...
	try {
		...
		const raced = await entity
			...
			.catch(() => null);
		if (Array.isArray(raced) && raced[0]?.id)
			return { id: raced[0].id, deduped: true };
		throw err;
	}
}

async function writeMetricToBase44(
	base44,
	evt,
	{ kind, ok, summaryExtra = null },
) {
...
if (args.check === true || args["config-check"] === true) {
		...
		try {
			...
			if (pathname === "/all-good") {
				...
				if (!ran.ok) {
					json(res, 503, {
						ok: false,
						error: ran.error ?? "all_good_failed",
			...
			if (pathname === "/acp/v1/transactions") {
				...
				if (!shouldCreatePayPalOrders()) {
					json(res, 403, {
						ok: false,
						error:
							"Order creation disabled (set PAYPAL_ENABLE_ORDER_CREATE=true)",
					});
					return;
				}
				try {
				...
				const amountRaw =
					picked?.price ?? picked?.amount ?? body.amount ?? body.total ?? null;
				...
				const currency = normalizeCurrency(
					picked?.currency ?? body.currency,
					String(process.env.PAYPAL_CURRENCY ?? "USD"),
				);
				const description =
					String(picked?.name ?? body.description ?? "").trim() || null;
			...
			const wantPayoutStatusWrite =
				shouldWritePayoutStatusFromPayPal() && !!payoutUpdate;
			const payoutBatchUpdate = mapPayPalWebhookToPayoutBatchUpdate(evt);
			const wantPayoutBatchWrite =
				shouldWritePayoutStatusFromPayPal() && !!payoutBatchUpdate;
			const base44 =
				wantWriteEvent ||
				wantMetrics ||
				wantRevenueWrite ||
				wantPayoutStatusWrite ||
				wantPayoutBatchWrite
			...
			json(res, 200, {
				...
				processingMs: Date.now() - startMs,
				summary: buildSummary(evt, {
					revenueCandidate: revenueEvent ? true : false,
				}),
			});
		} catch (e) {
			if (shouldAlertOnWebhookErrors()) {
	...
	server.listen(port, () => {
		...
		process.stdout.write(
			`${JSON.stringify({ ok: true, listening: true, port, path: "/paypal/webhook", localUrl, healthUrl })}\n`,
		);
		const publicBase = (
