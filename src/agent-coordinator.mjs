...
async function ensureReadiness() {
	...
	const okEnv =
		live && payoutWrite && appId && token && paypalApproved && paypalSend;
	if (!okEnv) {
	...
	if (!schemasOk) {
		log(
			"readiness: schema bootstrap failed, enabling offline mode and queuing",
		);
		process.env.BASE44_OFFLINE = "true";
		return { ok: false, offline: true };
	}
	try {
		...
		for (const name of entities) {
			...
			await e.list("-created_date", 1, 0).catch(() => {
				throw new Error(`entity_missing:${name}`);
			});
		}
...
function getBase44ApiUrl() {
	const u = String(
		process.env.BASE44_API_URL ?? process.env.BASE44_SERVER_URL ?? "",
	).trim();
...
function inferFieldType(field) {
	...
	if (f.includes("date") || f.includes("time") || f.includes("occurred"))
		return "text";
	if (f.includes("notes") || f.includes("metadata") || f.includes("payload"))
		return "json";
...
async function ensureSchemaBootstrap() {
	...
	const batch = {
		...
		fields: batchMap
			? buildFieldsFromMap(batchMap, [
					batchMap.batchId,
					batchMap.totalAmount,
					batchMap.currency,
					batchMap.status,
				]).map((f) =>
	...
	const item = {
		...
		fields: itemMap
			? buildFieldsFromMap(itemMap, [
					...
					itemMap.currency,
					itemMap.recipient,
					itemMap.recipientType,
				]).map((f) => (f.name === itemMap.itemId ? { ...f, unique: true } : f))
			: [
	...
	const tx = {
		...
		fields: txMap
			? buildFieldsFromMap(txMap, [
					txMap.transactionType,
					txMap.amount,
					txMap.description,
					txMap.transactionDate,
				])
			: [
...
async function publishGitChangesetsOnce() {
	if (!envTrue(process.env.BASE44_ENABLE_CHANGESET_WRITE))
		return { ok: false, skipped: true };
	const _base44 = buildBase44ServiceClient();
...
function writeSwarmAgents(agents) {
	const dir = path.resolve("data/swarm");
	ensureDir(dir);
	const file = path.join(dir, "agents.json");
	// Merge with existing agents by id
	let existing = [];
	try {
		if (fs.existsSync(file)) {
			const txt = fs.readFileSync(file, "utf8");
			const json = JSON.parse(txt);
			existing = Array.isArray(json?.agents) ? json.agents : [];
		}
	} catch {}
	const byId = new Map();
	...
	for (const a of Array.isArray(agents) ? agents : []) {
		const id = String(a?.id ?? "").trim() || `agent_${Date.now()}`;
		const existing = byId.get(id);
		byId.set(id, { ...existing, ...a });
	}
	const merged = Array.from(byId.values());
	const payload = { agents: merged };
	fs.writeFileSync(file, JSON.stringify(payload, null, 2));
	return file;
}

function readBase44Agents() {
	try {
		const file = path.resolve("data/base44/agents.json");
...
function toSwarmAgentShape(list) {
...
function syncBase44AgentsOnce() {
	const res = spawnSync(process.execPath, ["scripts/sync-base44-agents.mjs"], {
		cwd: process.cwd(),
		encoding: "utf8",
	});
...
async function ingestAgentsIntoSwarmOnce() {
	try {
		const sync = syncBase44AgentsOnce();
		if (!sync.ok) return { ok: false, reason: "sync_failed" };
		const raw = readBase44Agents();
		const mapped = toSwarmAgentShape(raw);
		const file = writeSwarmAgents(mapped);
		log(`coordinator: agents synced=${mapped.length} path=${file}`);
		return { ok: true, count: mapped.length, file };
	} catch (e) {
...
async function syncPayPalBatchesOnce() {
	if (!envTrue(process.env.BASE44_ENABLE_PAYOUT_LEDGER_WRITE))
		return { ok: false, skipped: true };
	const _base44 = buildBase44ServiceClient();
	const entity = base44.asServiceRole.entities["PayoutBatch"];
	const list = await entity
		.list("-created_date", 250, 0, ["id", "batch_id", "notes"])
		.catch(() => []);
	const ids = [];
	...
	for (const batchId of unique) {
		const child = spawn(
			"node",
			["./src/sync-paypal-payout-batch.mjs", "--batch-id", batchId],
			{
				stdio: ["ignore", "inherit", "inherit"],
				cwd: process.cwd(),
			},
...
function startChangeWatcher({ debounceMs = 5000 } = {}) {
	...
	const trigger = () => {
		...
		timer = setTimeout(
			() => publishGitChangesetsOnce().catch(() => {}),
			debounceMs,
		);
	};
...
async function pollBase44DirectivesOnce() {
	if (!envTrue(process.env.SWARM_ENABLE_DIRECTIVES))
		return { ok: false, skipped: true };
	const base44 = buildBase44ServiceClient();
	const names = [
		String(process.env.BASE44_DIRECTIVE_ENTITY ?? "SwarmDirective"),
		"Mission",
	];
	let picked = null;
	...
	const directives = await picked
		.list("-created_date", 50, 0, ["id", "action", "metadata"])
		.catch(() => []);
	for (const d of Array.isArray(directives) ? directives : []) {
		...
		const replicas = Number(
			d?.metadata?.replicas ?? process.env.SWARM_REPLICAS ?? "0",
		);
...
function ensureAcpProducts() {
	...
	const products = [
		{
			id: "rwcerts_basic",
			name: "Real World Certs Basic",
			price: 49,
			currency: "USD",
		},
		{
			id: "rwcerts_pro",
			name: "Real World Certs Pro",
			price: 149,
			currency: "USD",
		},
		{
			id: "rwcerts_enterprise",
			name: "Real World Certs Enterprise",
			price: 499,
			currency: "USD",
		},
	];
...
async function pollSalesHealthOnce() {
	...
	const list = await revenue
		.list("-created_date", 250, 0, ["id", "occurred_at"])
		.catch(() => []);
...
async function main() {
	...
	await ingestAgentsIntoSwarmOnce().catch(() => {});
	...
	ensureAcpProducts();
	const _webhook = startWebhookServer();
	await settlementDaemon.startAutoSettlementDaemon();
	const changesetIntervalMs = Number(
		args.changesetIntervalMs ??
			process.env.CHANGESET_PUBLISH_INTERVAL_MS ??
			"300000",
	);
	const syncIntervalMs = Number(
		args.syncIntervalMs ?? process.env.PAYPAL_SYNC_INTERVAL_MS ?? "600000",
	);
	const directivesIntervalMs = Number(
		process.env.SWARM_DIRECTIVES_INTERVAL_MS ?? "300000",
	);
	...
	startChangeWatcher({
		debounceMs: Number(process.env.CHANGESET_DEBOUNCE_MS ?? "5000") || 5000,
	});
	setInterval(
		() => publishGitChangesetsOnce().catch(() => {}),
		Math.max(60000, changesetIntervalMs),
	);
	setInterval(
		() => syncPayPalBatchesOnce().catch(() => {}),
		Math.max(60000, syncIntervalMs),
	);
	setInterval(
		() => pollBase44DirectivesOnce().catch(() => {}),
		Math.max(60000, directivesIntervalMs),
	);
	setInterval(
		() => pollSalesHealthOnce().catch(() => {}),
		Math.max(60000, Number(process.env.SALES_AUDIT_INTERVAL_MS ?? "900000")),
	);
}
