...
function loadIntentEnvelope(args) {
	if (args["intent-envelope"])
		return JSON.parse(String(args["intent-envelope"]));
	if (args["intent-envelope-file"])
		return readJsonFile(String(args["intent-envelope-file"]));
	...
	throw new Error(
		"Missing intent (use --intent-envelope/--intent-envelope-file or --intent-payload/--intent-payload-file)",
	);
}
...
function enforceIntentConstraints(intentPayload, { currency, total }) {
	...
	if (wanted && wanted !== currency)
		throw new Error(
			`Intent currency mismatch (want ${wanted}, got ${currency})`,
		);
	...
	if (maxRaw != null) {
		...
		if (Number.isFinite(max) && total > max)
			throw new Error(`Intent max_amount exceeded (${total} > ${max})`);
	}
}
...
async function filterAlreadySettled(base44, items, { enabled }) {
	...
	for (const it of items) {
		const settled = await isRevenueExternalIdSettled(
			base44,
			cfg,
			it.revenue_external_id,
		);
...
async function main() {
	...
	const holder = String(
		args.holder ?? process.env.SWARM_INSTANCE_ID ?? `local:${process.pid}`,
	);
	...
	if (!verified.ok) {
		process.stdout.write(
			`${JSON.stringify({ ok: false, error: "Invalid intent mandate", violations: verified.violations })}\n`,
		);
		process.exitCode = 1;
		return;
	}
	...
	if (intentPayload?.type !== "ap2.intent") {
		throw new Error(
			`Intent mandate must have type=ap2.intent (got ${String(intentPayload?.type ?? "")})`,
		);
	}

	const pospProof = calculatePosp({
		agentId: holder,
	...
	if (!pospCheck.ok) {
		process.stdout.write(
			`${JSON.stringify({ ok: false, error: "posp_insufficient", posp: pospCheck })}\n`,
		);
		process.exitCode = 1;
		return;
	}
	...
	if (!egressCheck.ok) {
		process.stdout.write(
			`${JSON.stringify({ ok: false, error: "egress_ip_mismatch", egress: egressCheck })}\n`,
		);
		process.exitCode = 1;
		return;
	}
	...
	const intentWrite = await writeBase44MandateIdempotent(
		base44,
		mandateCfg,
		intentEnvelope,
		{
	...
	const lease = dryRun
		...
		: await acquireWorkLease(base44, leaseCfg, {
				key: leaseKey,
				holder,
				ttlMs: leaseTtlMs,
				meta: { intent_id: intentPayload.id },
			});
	...
	const currency =
		intentPayload?.constraints?.currency ?? revenueCfg.defaultCurrency;
	...
	const mppDid = normalizeDid(
		process.env.AP2_MPP_DID,
		"did:swarm:mpp:bank_rails",
	);
	...
	if (!quoteVerify.ok)
		throw new Error(
			`Quote signing failed: ${quoteVerify.violations.join(",")}`,
		);
	const quoteWrite = await writeBase44MandateIdempotent(
		base44,
		mandateCfg,
		quoteEnvelope,
		{
	...
	paymentPayload.action.data.metadata.intent_payload_hash =
		mandatePayloadHash(intentPayload);
	paymentPayload.action.data.metadata.quote_payload_hash =
		mandatePayloadHash(quotePayload);

	const paymentEnvelope = signMandatePayload(paymentPayload, {
		kid: mandateKid,
	});
	const paymentVerify = verifyMandateEnvelope(paymentEnvelope);
	if (!paymentVerify.ok)
		throw new Error(
			`Payment signing failed: ${paymentVerify.violations.join(",")}`,
		);
	const paymentWrite = await writeBase44MandateIdempotent(
		base44,
		mandateCfg,
		paymentEnvelope,
		{
	...
	const payoutCreated = await createBase44PayoutRequestIdempotent(
		base44,
		payoutCfg,
		payoutPayload,
		{ dryRun },
