...
async function main() {
	...
	const beneficiary =
		args.beneficiary ?? process.env.EARNING_BENEFICIARY ?? null;
	const wantedCurrency = args.currency ?? process.env.EARNING_CURRENCY ?? null;
	const status =
		args.status ?? process.env.EARNING_STATUS ?? "settled_externally_pending";
	...
	const limit =
		Number(args.limit ?? process.env.EARNING_LIST_LIMIT ?? "500") || 500;
	...
	if (beneficiary)
		items = items.filter(
			(it) => String(it.beneficiary ?? "") === String(beneficiary),
		);
	if (status)
		items = items.filter((it) => String(it.status ?? "") === String(status));
	if (wantedCurrency)
		items = items.filter(
			(it) => String(it.currency ?? "") === String(wantedCurrency),
		);
	...
	const currency =
		wantedCurrency ??
		items[0]?.currency ??
		process.env.BASE44_DEFAULT_CURRENCY ??
		"USD";
	...
	const created = await createBase44ExternalSettlementIdempotent(
		base44,
		settlementCfg,
		settlement,
		{ dryRun },
