...
function buildReadinessSummary() {
	...
	const paypalConfigured =
		hasRealEnv("PAYPAL_CLIENT_ID") && hasRealEnv("PAYPAL_CLIENT_SECRET");
	...
	const flags = {
		...
		BASE44_ENABLE_REVENUE_FROM_PAYPAL: getEnvBool(
			"BASE44_ENABLE_REVENUE_FROM_PAYPAL",
			false,
		),
		BASE44_ENABLE_PAYPAL_PAYOUT_STATUS_WRITE: getEnvBool(
			"BASE44_ENABLE_PAYPAL_PAYOUT_STATUS_WRITE",
			false,
		),
		BASE44_ENABLE_PAYOUT_LEDGER_WRITE: getEnvBool(
			"BASE44_ENABLE_PAYOUT_LEDGER_WRITE",
			false,
		),
		BASE44_ENABLE_TRUTH_ONLY_UI: getEnvBool(
			"BASE44_ENABLE_TRUTH_ONLY_UI",
			false,
		),
		BASE44_ENABLE_PAYPAL_METRICS: getEnvBool(
			"BASE44_ENABLE_PAYPAL_METRICS",
			false,
		),
		PAYPAL_PPP2_APPROVED:
			getEnvBool("PAYPAL_PPP2_APPROVED", false) ||
			getEnvBool("PPP2_APPROVED", false),
		PAYPAL_PPP2_ENABLE_SEND:
			getEnvBool("PAYPAL_PPP2_ENABLE_SEND", false) ||
			getEnvBool("PPP2_ENABLE_SEND", false),
	};

	const payoutWriteAllowed = flags.BASE44_ENABLE_PAYOUT_LEDGER_WRITE && live;
	const paypalSendEnabled =
		(flags.PAYPAL_PPP2_APPROVED && flags.PAYPAL_PPP2_ENABLE_SEND) ||
		String(process.env.AUTONOMOUS_ALLOW_PAYPAL_PAYOUTS ?? "").toLowerCase() ===
			"true" ||
		String(process.env.BASE44_ALLOW_PAYPAL_PAYOUTS ?? "").toLowerCase() ===
			"true";

	const warnings = [];
	const paypalSandbox =
		paypalMode === "sandbox" ||
		paypalApiBaseUrl.toLowerCase().includes("sandbox.paypal.com");
	if (live && paypalSandbox) {
		warnings.push("SWARM_LIVE=true but PayPal is configured for sandbox");
	}
	if (flags.BASE44_ENABLE_PAYOUT_LEDGER_WRITE && !live) {
		warnings.push("Payout ledger writes enabled but SWARM_LIVE=false");
	}
	...
	if (live && paypalSandbox)
		missing.push("PAYPAL_MODE/PAYPAL_API_BASE_URL (must be live)");

	if (live && (!flags.PAYPAL_PPP2_APPROVED || !flags.PAYPAL_PPP2_ENABLE_SEND)) {
		warnings.push(
			"PayPal PPP2 flags not fully enabled (PAYPAL_PPP2_APPROVED and PAYPAL_PPP2_ENABLE_SEND)",
		);
	}
	...
	if (!paypalConfigured)
		nextSteps.push("Set PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET");
	...
	if (warnings.length)
		nextSteps.push(
			'PowerShell: $env:PAYPAL_MODE="live" (or unset PAYPAL_API_BASE_URL sandbox)',
		);
	if (!payoutWriteAllowed)
		nextSteps.push(
			'PowerShell: $env:BASE44_ENABLE_PAYOUT_LEDGER_WRITE="true"; $env:SWARM_LIVE="true"',
		);
	if (!paypalSendEnabled)
		nextSteps.push(
			'PowerShell: $env:PAYPAL_PPP2_APPROVED="true"; $env:PAYPAL_PPP2_ENABLE_SEND="true"',
		);
...
function getMissionHealthConfig() {
	return {
		...
		autoDeployEnabled: getEnvBool("BASE44_ENABLE_MISSION_AUTO_DEPLOY", false),
		truthOnlyUi: getEnvBool("BASE44_ENABLE_TRUTH_ONLY_UI", false),
		deployableThreshold:
			Number(process.env.BASE44_MISSION_HEALTH_DEPLOYABLE_THRESHOLD ?? "0.8") ||
			0.8,
		entities: {
			mission: process.env.BASE44_MISSION_ENTITY ?? "Mission",
			webhook:
				process.env.BASE44_MISSION_HEALTH_WEBHOOK_ENTITY ??
				"PayPalWebhookEvent",
			revenue:
				process.env.BASE44_MISSION_HEALTH_REVENUE_ENTITY ?? "RevenueEvent",
		...
		fields: {
			mission: {
				...
				healthScore:
					process.env.BASE44_MISSION_FIELD_HEALTH_SCORE ?? "health_score",
				healthProofs:
					process.env.BASE44_MISSION_FIELD_HEALTH_PROOFS ?? "health_proofs",
				lastHealthCheck:
					process.env.BASE44_MISSION_FIELD_LAST_HEALTH_CHECK ??
					"last_health_check",
			},
			webhook: {
...
function classifyMission(mission, cfg) {
	...
	if (t.includes("financial") || t.includes("revenue") || t.includes("payout"))
		return "financial_mission";
...
function computeHealthScore(classKey, proofs) {
	const weights =
		classKey === "financial_mission"
			? {
					webhook_heartbeat: 0.4,
					required_events: 0.2,
					ledger_activity: 0.3,
					paypal_sync: 0.1,
				}
			: {
					webhook_heartbeat: 0.2,
					required_events: 0.0,
					ledger_activity: 0.6,
					paypal_sync: 0.2,
				};
...
function computeDeployable({
	...
	if (classKey === "financial_mission") {
		...
		if (
			Number(requirements?.min_ledger_writes ?? 0) > 0 &&
...
function summarizeMissionHealthResults(results) {
	...
	for (const r of list) {
		...
		if (Number.isFinite(score)) {
			summary.minHealthScore =
				summary.minHealthScore == null
					? score
					: Math.min(summary.minHealthScore, score);
		}

		const classKey = String(r.classKey ?? "unknown");
		if (!summary.byClassKey[classKey]) {
			summary.byClassKey[classKey] = {
				total: 0,
				deployable: 0,
				notDeployable: 0,
			};
		...
		for (const p of proofs) {
			...
			summary.proofStatusCounts[key] =
				(summary.proofStatusCounts[key] ?? 0) + 1;
		}
	}
...
async function calculateMissionHealth(base44, cfg, mission) {
	...
	try {
		const webhook = await webhookEntity.filter(
			...
			0,
			[
				cfg.fields.webhook.eventId,
				cfg.fields.webhook.eventType,
				cfg.fields.webhook.at,
				"created_date",
				"created_at",
				"at",
			],
	...
	if (webhookErr) {
		...
		const at = pickFirst(lastWebhook, [
			cfg.fields.webhook.at,
			"created_date",
			"created_at",
			"at",
	...
	const requiredEvents = Array.isArray(requirements?.required_events)
		? requirements.required_events
		: [];
		...
		if (webhookErr) {
			...
			const types = new Set(
				(Array.isArray(sample) ? sample : [])
					.map((r) =>
						pickFirst(r, [cfg.fields.webhook.eventType, "event_type", "type"]),
					)
					.filter((x) => x != null)
					.map((x) => String(x)),
			);
	...
	try {
		const revenue = await revenueEntity.filter(
			...
			0,
			[
				cfg.fields.revenue.eventId,
				cfg.fields.revenue.amount,
				cfg.fields.revenue.at,
				"created_date",
				"occurred_at",
				"at",
			],
	...
	if (revenueErr) {
		...
		proofs.push({
			...
			status: "active",
			last_event_id:
				pickFirst(lastRevenue, [
					cfg.fields.revenue.eventId,
					"event_id",
					"id",
				]) ?? null,
			amount:
				pickFirst(lastRevenue, [cfg.fields.revenue.amount, "amount"]) ?? null,
			at:
				pickFirst(lastRevenue, [
					cfg.fields.revenue.at,
					"occurred_at",
	...
	try {
		const metric = await metricEntity.filter(
			{
				[cfg.fields.metric.kind]: "sync_payout_batch",
				[cfg.fields.metric.ok]: true,
			},
			"-created_date",
			1,
			0,
	...
	if (metricErr) {
		...
		const at =
			pickFirst(lastMetric, [
				cfg.fields.metric.at,
				"at",
				"created_date",
				"created_at",
			]) ?? null;
		...
		const maxH =
			Number(
				process.env.BASE44_MISSION_HEALTH_MAX_SYNC_STALENESS_HOURS ?? "24",
			) || 24;
		proofs.push({
			type: "paypal_sync",
	...
	const metadata = coerceObject(
		pickFirst(mission, [cfg.fields.mission.metadata, "metadata"]),
	);
	const nextMetadata = {
		...metadata,
		evidence_gated: true,
	...
	if (cfg.enableWrite) {
		...
		const allowAutoDeploy = cfg.autoDeployEnabled && cfg.truthOnlyUi;
		if (allowAutoDeploy && deployable) {
			patch[cfg.fields.mission.deployed] = true;
		}
		try {
			await missionEntity.update(String(missionId), patch);
		} catch {
			await missionEntity.update(String(missionId), {
				[cfg.fields.mission.metadata]: nextMetadata,
			});
		}
	}
...
async function runMissionHealthOnce(base44, cfg, { missionId, limit }) {
	const missionEntity = base44.asServiceRole.entities[cfg.entities.mission];
	const fields = [
		cfg.fields.mission.id,
		cfg.fields.mission.type,
		cfg.fields.mission.status,
		cfg.fields.mission.metadata,
		cfg.fields.mission.deployed,
	];
	let missions = [];
	if (missionId) {
		missions = await missionEntity.filter(
			{ [cfg.fields.mission.id]: String(missionId) },
			"-updated_date",
			1,
			0,
			fields,
...
async function main() {
	...
	const missionHealth =
		args["mission-health"] === true || args["mission-health-check"] === true;
	const missionHealthSummary =
		args.summary === true || args["mission-health-summary"] === true;

	const intervalMs = getIntervalMs();
	const enableWrite =
		(process.env.BASE44_ENABLE_HEALTH_WRITE ?? "false").toLowerCase() ===
		"true";
	const enableAlerts =
		(process.env.BASE44_ENABLE_ALERTS ?? "false").toLowerCase() === "true";
	...
	if (readiness) {
		...
		const base44Ping = base44Client
			? await checkBase44(base44Client)
			: { ok: false, error: base44ClientErr ?? "Missing Base44 credentials" };
	...
	if (missionHealth) {
		...
		process.stdout.write(
			`${JSON.stringify({
				...
				missionHealth: {
					...
					deployableThreshold: cfg.deployableThreshold,
					missionId,
					limit: limit ?? null,
				},
			})}\n`,
		...
		while (true) {
			const at = nowIso();
			const results = await runMissionHealthOnce(base44, cfg, {
				missionId,
				limit,
			});
			...
			if (once) {
				...
				if (!Array.isArray(results) || results.length === 0) {
					process.stdout.write(
						`${JSON.stringify({ ok: true, at, note: "no missions matched criteria" })}\n`,
						() => process.exit(0),
					);
				}
				break;
	...
	process.stdout.write(
		`${JSON.stringify({ ok: true, monitoring: true, intervalMs, once })}\n`,
	);

	let lastAlertAt = 0;
	const alertCooldownMs =
		Number(process.env.ALERT_COOLDOWN_MS ?? "900000") || 900000;
