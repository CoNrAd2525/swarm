...
export class RouteManager {
	constructor({
		...
		baseBackoffMs = Number(process.env.ROUTE_BACKOFF_BASE_MS ?? "30000") ||
			30000,
		maxBackoffMs = Number(process.env.ROUTE_BACKOFF_MAX_MS ?? "3600000") ||
			3600000,
		weights = {},
		disabled = new Set(),
	} = {}) {
		const list =
			Array.isArray(routes) && routes.length > 0
				? routes
				: parseListEnv("ROUTE_LIST", []);
		const w = Object.assign(
			{},
			parseJsonEnv("ROUTE_WEIGHTS_JSON", {}),
			weights || {},
		);
		const dis = new Set(
			[
				...(parseJsonEnv("ROUTE_DISABLE_JSON", []) || []),
				...Array.from(disabled || []),
			].map(String),
		);
		...
		this.routes = list
			.map((name, index) => ({
				name,
				weight: Number(w[name] ?? 0) || 0,
				index,
			}))
			.filter((r) => !dis.has(r.name));
	...
	getAvailableRoutes() {
		...
		cooling.sort(
			(a, b) => (a.health.cooldownUntil || 0) - (b.health.cooldownUntil || 0),
		);
	...
	pickRoute() {
		...
		if (cooling.length > 0)
			return { route: cooling[0].name, info: cooling[0], cooling: true };
	...
	async withFailover(executor, { maxTries = null, onAttempt } = {}) {
		...
		if (ordered.length === 0) {
			return { ok: false, error: "no_routes_available", tried: [] };
		}
		...
		return {
			ok: false,
			error: lastError?.message ?? String(lastError || "all routes failed"),
