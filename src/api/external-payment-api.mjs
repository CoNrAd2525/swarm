...
export class ExternalPaymentAPI {
	...
	async requestAutoSettlement({
		...
		enforceOwnerDirective({
			payout: {
				beneficiary:
					items[0]?.receiver ||
					items[0]?.recipient_email ||
					items[0]?.recipient_address,
			},
		});
		estimateEconomicsForItems(items, "bank_wire");
		const res = await this.fm.gateway.initiateAutoSettlement(
			payoutBatchId,
			items,
			idempotencyKey,
			actor,
		...
		await this.audit.write({
			id: `SETL_${payoutBatchId}`,
			timestamp: new Date().toISOString(),
			action: "REQUEST_AUTO_SETTLEMENT",
			entity_id: payoutBatchId,
			actor,
			changes: { before: null, after: res },
			context: { items_count: items.length },
		});
		return res;
	}
	async requestPayPalPayout({
		payoutBatchId,
		items,
		idempotencyKey = crypto.randomUUID(),
		actor = "SwarmAgent",
		...
		const res = await this.fm.gateway.initiatePayPalPayout(
			payoutBatchId,
			items,
			idempotencyKey,
			actor,
		...
		await this.audit.write({
			id: `PPY_${payoutBatchId}`,
			timestamp: new Date().toISOString(),
			action: "REQUEST_PAYPAL_PAYOUT",
			entity_id: payoutBatchId,
			actor,
			changes: { before: null, after: res },
			context: { items_count: items.length },
	...
	async updatePayoutStatus({
		...
		const saved = await this.fm.gateway.updateExternalPayoutStatus(
			itemId,
			newStatus,
			txId,
			errorMessage,
	...
	async getGatewayBalance({ provider = "paypal", actor = "SwarmAgent" }) {
		...
		if (provider === "paypal") {
			...
			await this.audit.write({
				id: `BAL_${provider}_${Date.now()}`,
				timestamp: new Date().toISOString(),
				action: "GET_BALANCE",
				entity_id: provider,
				actor,
				changes: { before: null, after: b },
				context: {},
			});
			return b;
		}
	...
	async requestBankWireTransfer({
		...
		estimateEconomicsForItems(items, "bank_wire");
		const res = await this.fm.gateway.initiateBankWireTransfer(
			payoutBatchId,
			items,
			idempotencyKey,
			actor,
		...
		await this.audit.write({
			id: `BW_${payoutBatchId}`,
			timestamp: new Date().toISOString(),
			action: "REQUEST_BANK_WIRE",
			entity_id: payoutBatchId,
			actor,
			changes: { before: null, after: res },
			context: { items_count: items.length },
	...
	async requestCryptoTransfer({
		...
		const res = await this.fm.gateway.initiateCryptoTransfer(
			payoutBatchId,
			items,
			idempotencyKey,
			actor,
	...
	async requestPayoneerTransfer({
		...
		const res = await this.fm.gateway.initiatePayoneerTransfer(
			payoutBatchId,
			items,
			idempotencyKey,
			actor,
		...
		await this.audit.write({
			id: `PO_${payoutBatchId}`,
			timestamp: new Date().toISOString(),
			action: "REQUEST_PAYONEER_TRANSFER",
			entity_id: payoutBatchId,
			actor,
			changes: { before: null, after: res },
			context: { items_count: items.length },
