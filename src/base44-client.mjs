...
function normalizeUrlBase(url) {
	...
	try {
		const u = new URL(s);
		const base = `${u.protocol}//${u.host}${u.pathname}`.replace(/\/+$/g, "");
		return base;
	} catch {
		return s.replace(/\/+$/g, "");
	}
...
function resolveServerUrl() {
	const explicit =
		process.env.BASE44_SDK_SERVER_URL ||
		process.env.BASE44_API_URL ||
		process.env.BASE44_SERVER_URL ||
		null;
	const normalized = normalizeUrlBase(explicit);
	return normalized || "https://api.base44.com";
}
...
function ensureEntityBucket(store, entityName) {
	...
	if (!Array.isArray(store.entities[entityName].records))
		store.entities[entityName].records = [];
...
export function createOfflineClient({ filePath }) {
	...
	async function getStore() {
		...
		if (!storePromise) {
			storePromise = (async () => {
				const loaded = await readJsonFile(filePath).catch(() => ({
					entities: {},
				}));
				if (!loaded || typeof loaded !== "object") return { entities: {} };
				if (!loaded.entities || typeof loaded.entities !== "object")
					loaded.entities = {};
				return loaded;
			})();
		}
	...
	function makeEntity(entityName) {
		return {
			...
			update: async (id, patch) => {
				...
				if (idx === -1)
					throw new Error(`Offline entity ${entityName} missing id=${id}`);
				const now = new Date().toISOString();
				bucket.records[idx] = {
					...bucket.records[idx],
					...patch,
...
function normalizeAppIdInput(value) {
	...
	const looksLikeUrl =
		/^https?:\/\//i.test(unwrapped) || unwrapped.includes("base44.app");
...
function parseApiKeyValue(raw) {
	...
	if (v.startsWith("{") && v.endsWith("}")) {
		try {
			...
			const appId = normalizeAppIdInput(
				obj?.appId ?? obj?.app_id ?? obj?.applicationId ?? obj?.application_id,
			);
			const serviceToken = coerceNonEmptyString(
				obj?.serviceToken ??
					obj?.service_token ??
					obj?.token ??
					obj?.apiKey ??
...
function getOnlineAuth() {
	const envAppIdRaw = process.env.BASE44_APP_ID;
	const envToken = coerceNonEmptyString(process.env.BASE44_SERVICE_TOKEN);
	const apiKeyRaw = process.env.BASE44_API_KEY;
	let appId = normalizeAppIdInput(envAppIdRaw);
	let serviceToken = envToken;
	if (!serviceToken && apiKeyRaw) {
		const parsed = parseApiKeyValue(apiKeyRaw);
		if (parsed.serviceToken) serviceToken = parsed.serviceToken;
		if (!appId && parsed.appId) appId = normalizeAppIdInput(parsed.appId);
	}
...
export function buildBase44Client({
	allowMissing = false,
	mode = "auto",
} = {}) {
	const wantOffline =
