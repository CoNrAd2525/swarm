...
function buildRevenueData(cfg, event) {
	...
	if (fieldMap.status && event.status != null)
		data[fieldMap.status] = event.status;
	if (fieldMap.payoutBatchId && event.payoutBatchId != null)
		data[fieldMap.payoutBatchId] = event.payoutBatchId;
	if (fieldMap.verificationProof && event.verificationProof != null)
		data[fieldMap.verificationProof] = event.verificationProof;
...
export async function createBase44RevenueEvent(
	base44,
	cfg,
	event,
	{ dryRun } = {},
...
export async function createBase44RevenueEventIdempotent(
	base44,
	cfg,
	event,
	{ dryRun } = {},
	...
	if (cfg.fieldMap.source && event.source != null)
		filter[cfg.fieldMap.source] = event.source;
