...
export class WiseGateway {
	...
	async _ensureProfileAndRecipient() {
		...
		if (!this.profileId) {
			const profilesResponse = await this.makeAuthenticatedRequest("/v1/profiles");
			const profiles = await profilesResponse.json();
			const profile =
				profiles.find((p) => p.type === "personal") || profiles[0] || null;
			if (!profile) throw new Error("WiseGateway: No profiles found on account");
			this.profileId = profile.id;
		}

		if (process.env.OWNER_WISE_RECIPIENT_ID) return;
		...
		if (!recipient && ownerEmail) {
			recipient = recipients.find((r) => r.details?.email === ownerEmail);
		}
	...
	async executeTransfer(transferDetails) {
		...
		const { payoutBatchId, actor, description } = transferDetails || {};
