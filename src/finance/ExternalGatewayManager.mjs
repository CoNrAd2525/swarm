...
import { getPayoutBatchDetails, getPayPalBalance } from "../paypal-api.mjs";
...
import { WiseGateway } from "./gateways/WiseGateway.mjs";
...
function buildOwnerVaultInstruction({ amount, currency, payoutBatchId }) {
	const n = Number(amount);
	if (!Number.isFinite(n) || n <= 0) {
		throw new Error("invalid_owner_vault_amount");
	}
	const tokenSymbol = String(currency || "USDT").toUpperCase();
	...
	const chain = String(
		process.env.OWNER_VAULT_CHAIN || process.env.CRYPTO_CHAIN || "BSC",
	)
		.toUpperCase()
		.trim();
	if (!to || !contractAddress) {
		throw new Error("missing_owner_vault_destination_or_contract");
	}
	const decimals =
	...
	const amountWei = parseUnits(String(n), decimals);
	const calldata = iface.encodeFunctionData("releaseToOwner", [
		to,
		amountWei,
...
export class ExternalGatewayManager {
	constructor(storage, auditLogger, executor) {
		...
		this.payoutGuardrail = {
			history: [],
			WINDOW_MS: 3600 * 1000, // 1 hour
			MAX_PER_WINDOW: 50, // Max 50 payouts/hr
			MAX_AMOUNT_VELOCITY: 10000, // Max $10k/hr
		};
	}

	checkPayoutVelocity(recipientItems) {
		const now = Date.now();
		// Prune old history
		this.payoutGuardrail.history = this.payoutGuardrail.history.filter(
			(h) => now - h.timestamp < this.payoutGuardrail.WINDOW_MS,
		);

		const currentCount = this.payoutGuardrail.history.length;
		const currentVol = this.payoutGuardrail.history.reduce(
			(sum, h) => sum + h.amount,
			0,
		);

		const newCount = recipientItems.length;
		const newVol = recipientItems.reduce(
			(sum, i) => sum + Number(i.amount || 0),
			0,
		);

		if (currentCount + newCount > this.payoutGuardrail.MAX_PER_WINDOW) {
			throw new Error(
				`Velocity Limit Exceeded: ${currentCount + newCount} payouts/hr (Limit: ${this.payoutGuardrail.MAX_PER_WINDOW})`,
			);
		}

		if (currentVol + newVol > this.payoutGuardrail.MAX_AMOUNT_VELOCITY) {
			throw new Error(
				`Velocity Limit Exceeded: $${currentVol + newVol}/hr (Limit: $${this.payoutGuardrail.MAX_AMOUNT_VELOCITY})`,
			);
		}

		// Tentatively record (will be rolled back if exec fails? No, simpler to just block)
		// We'll record AFTER successful initiation in the main flow, or assume intent counts against quota to be safe.
		// Let's count it as intent.
		for (const i of recipientItems) {
			this.payoutGuardrail.history.push({
				timestamp: now,
	...
	async ensureCryptoGasCapacity(chain, getBalanceFn) {
		const key = String(chain || "").toUpperCase();
		const min = this.minGasConfig[key];
		if (!min || !getBalanceFn) return { ok: true };
		const balance = await getBalanceFn(key);
		if (balance == null) return { ok: false, reason: "UNKNOWN_BALANCE" };
		if (Number(balance) < Number(min)) {
			return {
				ok: false,
		...
		return { ok: true, balance, chain: key };
	}
	...
	async initiatePayPalPayout(
		...
		return this.executor.execute(
			...
			async () => {
				console.log(
					`💸 [GATEWAY] Initiating PayPal Payout Batch: ${payoutBatchId}`,
				);
				...
				// Velocity Check
				this.checkPayoutVelocity(recipientItems);
				...
				if (shouldAvoidPayPal()) {
					...
					const cryptoResult =
						await this.cryptoGateway.executeTransfer(cryptoTx);
					this.audit.log(
					...
					return {
						status:
							cryptoResult.status === "prepared" ? "processing" : "processing",
	 ...
	 */
	async updateExternalPayoutStatus(
		payoutItemId,
		newStatus,
		...
		this.audit.log(
			...
			saved,
			actor,
			{ newStatus, externalTransactionId },
		);
	 ...
	 */
	async initiateBankWireTransfer(
		payoutBatchId,
		recipientItems,
		idempotencyKey,
		actor = "System",
	...
	async initiateBinanceCryptoBoxTransfer(
		payoutBatchId,
		recipientItems,
		idempotencyKey,
		actor = "System",
		...
		const context = {
			action: "INITIATE_BINANCE_CRYPTOBOX",
			actor,
			payoutBatchId,
		};
		return this.executor.execute(
			...
			async () => {
				...
				return {
					...
					payout_batch_id: payoutBatchId,
					processed_at: new Date().toISOString(),
					route_attempted: "binance_cryptobox",
				};
			},
			context,
		);
	 ...
	 */
	async initiatePayoneerTransfer(
		payoutBatchId,
		recipientItems,
		idempotencyKey,
		actor = "System",
		...
		const context = {
			action: "INITIATE_PAYONEER_TRANSFER",
			actor,
			payoutBatchId,
		};
		return this.executor.execute(
	...
	async initiateStripeTransfer(
		...
		const context = {
			action: "INITIATE_STRIPE_TRANSFER",
			actor,
			payoutBatchId,
		};
		return this.executor.execute(
			...
			async () => {
				...
				const tx = recipientItems.map((item) => ({
					...
					destination:
						item.recipient_address ||
						item.crypto_address ||
						process.env.OWNER_TRON_USDT_ADDRESS,
					reference: item.note || `Batch ${payoutBatchId}`,
				}));
	...
	async initiateAutoSettlement(
		...
		const context = {
			action: "INITIATE_AUTO_SETTLEMENT",
			actor,
			payoutBatchId,
		};
		return this.executor.execute(
			...
			async () => {
				...
				// Force Wise route if environment variables are set
				const forceWise =
				...
				this.audit.log(
					...
					payoutBatchId,
					null,
					{ routes, forceWise, currency: currency0, amount: Number(amount0) },
					actor,
				);
				...
				const attempt = async ({ route }) => {
					try {
						...
						this.audit.log(
							...
							payoutBatchId,
							null,
							{ route, error: e?.message || String(e) },
							actor,
						);
						throw e;
					}
				...
				if (!res.ok) {
					this.audit.log(
						...
						payoutBatchId,
						null,
						{ tried: res.tried, error: res.error },
						actor,
					);
	...
	async broadcastSettlement(route, prepared, transactions, actor = "System") {
		if (route === "bank_transfer") {
			...
			this.audit.log(
				"BANK_WIRE_FILE_READY",
				prepared?.payout_batch_id || null,
				null,
				r,
				actor,
			);
	...
	async getPayPalPayoutBatchStatus(payoutBatchId, actor = "System") {
		try {
			...
			this.audit.log(
				"QUERY_PAYPAL_PAYOUT_STATUS_FAILED",
				payoutBatchId,
				null,
	...
	async listOwnerVaultInstructions(actor = "System") {
		const dir = path.join(process.cwd(), "settlements", "owner_vault");
		try {
			if (!fs.existsSync(dir)) {
				const result = { status: "not_found", files: [] };
				this.audit.log("OWNER_VAULT_LIST", null, null, result, actor);
				return result;
			}
			const files = fs
				.readdirSync(dir)
				.filter(
					(f) =>
						f.startsWith("owner_vault_") &&
						(f.endsWith(".json") || !f.includes(".")),
				);
			const entries = [];
			...
			const result = { status: "ok", files: entries };
			this.audit.log("OWNER_VAULT_LIST", null, null, result, actor);
			return result;
		} catch (e) {
			const err = {
			...
			this.audit.log("OWNER_VAULT_LIST_FAILED", null, null, err, actor);
			return err;
		}
	}

	async getCryptoStatus(
		{ address, amount, provider = "binance" },
		actor = "System",
	) {
		try {
			const r = await this.cryptoGateway.getWithdrawalStatus({
				provider,
				address,
				amount,
			});
	...
	async getSettlementStatus(route, ref, actor = "System") {
		...
		if (r === "smart_contract_owner")
			return this.listOwnerVaultInstructions(actor);
		if (r === "crypto") {
		...
		if (r === "tron") {
			...
			const amt = typeof ref === "object" ? ref.amount : null;
			try {
				const res = await this.tronGateway.checkIncoming({
					address: addr,
					minAmount: amt || 0,
				});
				this.audit.log(
				...
				if (res.status === "RECEIVED") {
					...
					const file = path.join(
						outDir,
						`crypto_tron_received_${Date.now()}.json`,
					);
					const receipt = {
						network: "TRON",
	...
	async initiatePlatformInstruction(
		payoutBatchId,
		providerId,
		recipientItems,
		idempotencyKey,
		...
		const context = {
			action: "INITIATE_PLATFORM_INSTRUCTION",
			actor,
			payoutBatchId,
			providerId,
		};
		return this.executor.execute(
			...
			async () => {
				...
				const result = await this.platformGateway.generate(
					providerId,
					tx,
					`Instruction for ${p.name}`,
				);
				this.audit.log(
					"PLATFORM_INSTRUCTIONS_READY",
					payoutBatchId,
					null,
					result,
					actor,
				...
				return {
					...
					payout_batch_id: payoutBatchId,
					processed_at: new Date().toISOString(),
					route_attempted: `platform:${providerId}`,
				};
			},
			context,
		);
	...
	subscribeSettlementStatus(route, ref, onUpdate, intervalMs = 2000) {
		const timer = setInterval(
			async () => {
				try {
					const s = await this.getSettlementStatus(
						route,
						ref,
						"AgentSubscriber",
					);
