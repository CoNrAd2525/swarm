...
export class AgentCommunicationProtocol extends EventEmitter {
	...
	async start() {
		...
		console.log(
			`📡 Agent communication protocol started (interval: ${this.processInterval}ms)`,
		);
	}
	...
	async sendMessage(fromAgentId, toAgentId, messageType, payload) {
		...
		console.log(
			`📤 Message sent: ${fromAgentId} -> ${toAgentId} (${messageType})`,
		);
	...
	async handleMessage(message) {
		try {
			console.log(
				`📨 Processing message: ${message.type} from ${message.from}`,
			);
			...
			console.error(
				`❌ Message handling error for ${message.type}:`,
				error.message,
			);
	...
	async handlePaymentRequest(message) {
		...
		if (paymentAgent) {
			...
			await this.sendMessage(message.to, paymentAgent.id, "process_payment", {
				amount,
				currency,
				recipient,
			...
			await this.sendMessage(message.to, message.from, "payment_failed", {
				request_id: message.id,
	...
	async handleSyncRequest(message) {
		...
		// Send sync response
		await this.sendMessage(message.to, message.from, "sync_response", {
			sync_type,
			status,
			agent_id: message.to,
		});
	...
	async handleCoordinationRequest(message) {
		...
		console.log(
			`🎯 Coordination request: ${action} for ${target_agents.length} agents`,
		);

		// Forward to target agents
		for (const targetAgentId of target_agents) {
			await this.sendMessage(message.to, targetAgentId, "coordination_action", {
				action,
				payload,
	...
	async cleanupOldMessages() {
		...
		if (filtered.length < messages.length) {
			...
			console.log(
				`🧹 Cleaned up ${messages.length - filtered.length} old messages`,
			);
		}
	}
}
