...
export class PaymentSyncAgent {
	...
	async start() {
		...
		console.log(`💰 Payment sync agent started (interval: ${this.syncInterval}ms)`);
	}
	...
	async checkWiseStatus() {
		try {
			...
			const response = await fetch(`https://api.wise.com/v1/profiles/${profileId}`, {
				headers: {
	...
	async checkBankingCircleStatus() {
		try {
			const apiKey = process.env.BANKING_CIRCLE_API_KEY || process.env.BC_API_KEY;
			...
			const response = await fetch('https://api.bankingcircle.com/v2/accounts', {
				headers: {
					'Authorization': `Bearer ${apiKey}`
				}
			});
	...
	async syncPendingSettlements() {
		try {
			...
			const pendingSettlements = payeeLinks.filter(link => 
				link.status === 'pending' || !link.status
			);
	...
	async processSettlement(settlement) {
		try {
			...
			// Find healthy rail
			const healthyRail = Object.entries(rails).find(([name, status]) => 
