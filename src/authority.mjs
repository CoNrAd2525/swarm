...
function getOwnerDeclarationFromEnv() {
	const ownerPayPal = normalizeEmailAddress(
		process.env.OWNER_PAYPAL_EMAIL ?? process.env.PAYPAL_EMAIL ?? null,
	);
	const ownerPayoneer = normalizeEmailAddress(
		process.env.OWNER_PAYONEER_ID ?? process.env.PAYONEER_EMAIL ?? null,
	);
...
export function computeAuthorityChecksum() {
	...
	const payload = {
		...
		base44PayoutDestinationJson: process.env.BASE44_PAYOUT_DESTINATION_JSON
			? String(process.env.BASE44_PAYOUT_DESTINATION_JSON)
			: null,
...
function enforceOwnerOnlyAllowlist({ owner, policy, action }) {
	...
	if (action && action.toLowerCase().includes("paypal")) {
		if (policy.paypal.size > 0) {
			if (!owner.ownerPayPal)
				throw new Error(
					"LIVE MODE NOT GUARANTEED (missing OWNER_PAYPAL_EMAIL)",
				);
			for (const email of policy.paypal) {
				if (email !== owner.ownerPayPal) {
					throw new Error(
						"LIVE MODE NOT GUARANTEED (paypal allowlist contains non-owner recipient)",
					);
				}
			}
		}
	...
	if (policy.payoneer.size > 0) {
		if (!owner.ownerPayoneer)
			throw new Error("LIVE MODE NOT GUARANTEED (missing OWNER_PAYONEER_ID)");
		for (const email of policy.payoneer) {
			if (email !== owner.ownerPayoneer) {
				throw new Error(
					"LIVE MODE NOT GUARANTEED (payoneer allowlist contains non-owner recipient)",
				);
			}
		}
	}
	...
	if (policy.bankWireAccounts.size > 0) {
		if (!owner.ownerBankAccount)
			throw new Error("LIVE MODE NOT GUARANTEED (missing OWNER_BANK_ACCOUNT)");
		for (const acct of policy.bankWireAccounts) {
			if (acct !== owner.ownerBankAccount) {
				throw new Error(
					"LIVE MODE NOT GUARANTEED (bank allowlist contains non-owner account)",
				);
			}
		}
	}
...
export function enforceAuthorityProtocol({
	...
	if (requireLive && !live)
		throw new Error(
			`LIVE MODE NOT GUARANTEED (${String(action ?? "authority")})`,
		);

	if (requireNoPlatformWallet && !getEnvBool("NO_PLATFORM_WALLET", false)) {
		throw new Error(
			`LIVE MODE NOT GUARANTEED (NO_PLATFORM_WALLET not true: ${String(action ?? "authority")})`,
		);
	}
	...
	if (
		...
		throw new Error(
			`LIVE MODE NOT GUARANTEED (offline mode enabled: ${String(action ?? "authority")})`,
		);
	}
	...
	if (requireOwnerDeclaration) {
		if (!owner.ownerPayPal && !owner.ownerPayoneer && !owner.ownerBankAccount) {
			throw new Error(
				`LIVE MODE NOT GUARANTEED (missing owner declaration: ${String(action ?? "authority")})`,
			);
		}
	}
...
export {
	getAllowedRecipientsPolicyFromEnv,
