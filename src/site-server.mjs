...
function start({ port = 8080 } = {}) {
	...
	app.get("/api/classroom/queue", async (req, res) => {
		try {
			...
			const requests = await readClassroomRequests({});
			const status = await readClassroomStatus({});
			const items = requests
				.slice(-200)
				.reverse()
				.map((r) => ({
					...r,
					status: status?.[r?.id] || null,
				}));
			res.json({ ok: true, items });
		} catch (e) {
			res.status(500).json({ ok: false, error: String(e?.message ?? e) });
	...
	app.post(
		...
		async (req, res) => {
			try {
				...
				const patch = {
					state: safeStr(req.body?.state || "", 40) || null,
					note: safeStr(req.body?.note || "", 240) || null,
				};
				const out = await setClassroomRequestStatus(id, patch, {});
