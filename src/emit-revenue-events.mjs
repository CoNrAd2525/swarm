...
async function scanSimulationArtifacts(base44, { limit = 200 } = {}) {
	...
	if (base44) {
		...
		try {
			...
			const missions = await missionEntity.list(
				"-updated_date",
		...
		try {
			const earningEntity =
				base44.asServiceRole.entities[earningCfg.entityName];
			const earnings = await earningEntity.list(
				"-created_date",
				Math.max(1, Math.floor(Number(limit ?? 200))),
				0,
			);
...
function shouldWriteChangeSets() {
	return (
		String(
			process.env.BASE44_ENABLE_CHANGESET_WRITE ?? "false",
		).toLowerCase() === "true"
	);
}
...
function getChangeSetConfigFromEnv() {
	...
	const chunkEntityName =
		process.env.BASE44_CHANGESET_CHUNK_ENTITY ?? "SwarmChangeSetChunk";

	const fieldMap = safeJsonParse(
		process.env.BASE44_CHANGESET_FIELD_MAP,
		null,
	) ?? {
	...
	const chunkFieldMap = safeJsonParse(
		process.env.BASE44_CHANGESET_CHUNK_FIELD_MAP,
		null,
	) ?? {
		changeSetId: "changeset_id",
		seq: "seq",
...
async function publishGitChangeSetsToBase44(base44, { repoRoots, dryRun }) {
	if (!base44)
		throw new Error(
			"Missing Base44 client; set BASE44_APP_ID/BASE44_SERVICE_TOKEN",
		);
	if (base44?.offline?.filePath)
		throw new Error(
			"Refusing changeset publish in offline mode (set BASE44_OFFLINE=false)",
		);
	if (!shouldWriteChangeSets())
		throw new Error(
			"Refusing changeset publish without BASE44_ENABLE_CHANGESET_WRITE=true",
		);
	...
	for (const rr of roots) {
		...
		const record = {
			...
			...(cfg.fieldMap.createdAt
				? { [cfg.fieldMap.createdAt]: createdAt }
				: {}),
		};
...
function verifyNoOfflineInLive() {
	if (
		envIsTrue(process.env.BASE44_OFFLINE, "false") ||
		envIsTrue(process.env.BASE44_OFFLINE_MODE, "false")
	) {
	...
	return {
		...
		endpoints: {
			...
			paypalApiBaseUrl: process.env.PAYPAL_API_BASE_URL
				? String(process.env.PAYPAL_API_BASE_URL)
				: "(default)",
			base44ServerUrl: process.env.BASE44_SERVER_URL
				? String(process.env.BASE44_SERVER_URL)
				: "(default)",
		},
	};
}
...
function shouldRelaxEvidence() {
	const v =
		process.env.BASE44_RELAX_HARD_EVIDENCE ??
...
function getEarningSharePct() {
	const raw =
		process.env.EARNING_SHARE_PCT ?? process.env.SWARM_EARNING_SHARE_PCT ?? "1";
...
function getEarningBeneficiary(args) {
	...
	const ownerPayPal = normalizeEmailAddress(
		getEnvFirst(["OWNER_PAYPAL_EMAIL", "OWNER_PAYPAL"]) ?? null,
	);
	return ownerPayPal ?? null;
}
...
function getSettlementArgs(args, { beneficiary, dryRun }) {
	...
	const currency =
		args["settlement-currency"] ?? args.settlementCurrency ?? null;
	...
	const markIssued =
		args["mark-issued"] === true ||
		args.markIssued === true ||
		args["settlement-mark-issued"] === true;
...
function buildDefaultSettlementOutPath({ settlementId, bankCsv }) {
	...
	const idPart = settlementId
		? String(settlementId).replace(/[^\w-]+/g, "_")
		: ts;
...
function runSettlementExportCli(settlement) {
	...
	if (settlement.beneficiary)
		args.push("--beneficiary", String(settlement.beneficiary));
	...
	if (settlement.settlementId)
		args.push("--settlement-id", String(settlement.settlementId));
	...
	const outPath = settlement.outPath
		? String(settlement.outPath)
		: buildDefaultSettlementOutPath(settlement);
...
function getPayoutBatchConfigFromEnv() {
	const entityName =
		process.env.BASE44_LEDGER_PAYOUT_BATCH_ENTITY ?? "PayoutBatch";
	const mapFromEnv = safeJsonParse(
		process.env.BASE44_LEDGER_PAYOUT_BATCH_FIELD_MAP,
		null,
	);
...
function getPayoutItemConfigFromEnv() {
	const entityName =
		process.env.BASE44_LEDGER_PAYOUT_ITEM_ENTITY ?? "PayoutItem";
	const mapFromEnv = safeJsonParse(
		process.env.BASE44_LEDGER_PAYOUT_ITEM_FIELD_MAP,
		null,
	);
	const fieldMap = mapFromEnv ?? {
		itemId: "item_id",
...
function shouldWritePayoutLedger() {
	return (
		(process.env.BASE44_ENABLE_PAYOUT_LEDGER_WRITE ?? "false").toLowerCase() ===
		"true"
	);
}
...
function shouldWritePayPalPayoutStatus() {
	return (
		(
			process.env.BASE44_ENABLE_PAYPAL_PAYOUT_STATUS_WRITE ?? "false"
		).toLowerCase() === "true"
	);
}
...
function sanitizeDestination(dest) {
	...
	return {
		...
		beneficiaryMasked: beneficiary
			? mask(beneficiary, { keepStart: 1, keepEnd: 0 })
			: "",
	};
}
...
function normalizeDestinationObject(dest) {
	...
	const beneficiary =
		dest?.beneficiary ??
		dest?.beneficiary_name ??
		dest?.beneficiaryName ??
		dest?.account_holder ??
		dest?.accountHolder ??
		"";
...
function shouldOptimizePaymentRouting() {
	const v =
		process.env.AUTONOMOUS_OPTIMIZE_PAYMENT_ROUTING ??
		process.env.BASE44_OPTIMIZE_PAYMENT_ROUTING ??
		"false";
	return String(v).toLowerCase() === "true";
}
...
function getRoutingUsdPayPalThreshold() {
	const v = Number(
		process.env.AUTONOMOUS_ROUTING_USD_PAYPAL_THRESHOLD ?? "5000",
	);
	if (!Number.isFinite(v) || v <= 0) return 5000;
	return v;
}
...
function chooseOptimizedRecipientType({ amount, currency, meta, beneficiary }) {
	...
	const paypalEmail = isPayPalPayoutSendEnabled()
		? normalizeEmailAddress(
				meta?.paypal_email ?? meta?.paypalEmail ?? beneficiary ?? null,
			)
		: null;
...
function destinationsEqual(a, b) {
	...
	return (
		aa.bank === bb.bank &&
		aa.swift === bb.swift &&
		aa.account === bb.account &&
		aa.beneficiary === bb.beneficiary
	);
}
...
function resolveRecipientAddress(recipientType, meta, fallback) {
	...
	if (t === "paypal")
		return normalizeEmailAddress(
			meta?.paypal_email ?? meta?.paypalEmail ?? fallback ?? null,
		);
	if (t === "payoneer")
		return normalizeEmailAddress(
			meta?.payoneer_id ?? meta?.payoneerId ?? fallback ?? null,
		);
	if (t === "bank_wire") {
		return normalizeRecipientLabel(
			meta?.bank_beneficiary_name ??
				meta?.bankBeneficiaryName ??
...
function addOwnerRecipientsToPolicy(policy) {
	const ownerPayPal = normalizeEmailAddress(
		getEnvFirst(["OWNER_PAYPAL_EMAIL", "OWNER_PAYPAL"]) ?? null,
	);
	...
	const configured =
		...
		policy.bankWireAccounts.size > 0;
	...
	if (json != null && String(json).trim()) {
		...
		if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
			...
			const payoneer = parseAllowedRecipientList(
				parsed.payoneer ?? parsed.payoneer_id ?? parsed.payoneerId ?? [],
			);
			const bankWireAccounts = parseAllowedRecipientList(
				parsed.bank_wire ??
					parsed.bankWire ??
					parsed.bank_wire_accounts ??
					parsed.bankWireAccounts ??
			...
			const policy = {
				paypal: new Set(
					paypal.map((x) => normalizeEmailAddress(x)).filter(Boolean),
				),
				payoneer: new Set(
					payoneer.map((x) => normalizeEmailAddress(x)).filter(Boolean),
				),
				bankWireAccounts: new Set(
			...
			const configured =
				...
				policy.payoneer.size > 0 ||
				policy.bankWireAccounts.size > 0;
			return addOwnerRecipientsToPolicy({ ...policy, configured });
		}
	}
...
function isBankWireDestinationAllowedByPolicy(destination, policy) {
	...
	if (!policy?.bankWireAccounts || policy.bankWireAccounts.size === 0)
		return false;
...
function makeProofOfLifeBatchId({
	recipientType,
	recipient,
	amount,
	currency,
...
async function listAll(
	entity,
	{ fields = null, pageSize = 200, sort = "-created_date" } = {},
) {
	const out = [];
	let offset = 0;
	for (;;) {
...
function requireTotpIfNeeded(totalAmount, { args }) {
	...
	if (!String(secret).trim()) {
		throw new Error(
			"PAYOUT_APPROVAL_TOTP_SECRET is required for large batch approval",
		);
	}
	...
	if (!/^\d{6,8}$/.test(code))
		throw new Error("Missing or invalid --totp code for large batch approval");
...
async function computeAvailableBalance(base44) {
	...
	const payoutBatchEntity =
		base44.asServiceRole.entities[payoutBatchCfg.entityName];
	const payoutItemEntity =
		base44.asServiceRole.entities[payoutItemCfg.entityName];
	...
	const revs = await listAll(revenueEntity, {
	...
	let totalPaidOut = 0;
	for (const r of revs) {
		const status = revenueCfg.fieldMap.status
			? r?.[revenueCfg.fieldMap.status]
			: null;
		const amount = Number(
			revenueCfg.fieldMap.amount ? r?.[revenueCfg.fieldMap.amount] : 0,
		);
	...
	const batches = await listAll(payoutBatchEntity, {
		fields: batchFields,
		pageSize: 250,
	});
	const committedBatchIds = new Set();
	for (const b of batches) {
		const status = payoutBatchCfg.fieldMap.status
			? b?.[payoutBatchCfg.fieldMap.status]
			: null;
		if (
			status === "approved" ||
			...
			const bid = payoutBatchCfg.fieldMap.batchId
				? b?.[payoutBatchCfg.fieldMap.batchId]
				: null;
	...
	const items = await listAll(payoutItemEntity, {
		fields: itemFields,
		pageSize: 250,
	});
	...
	for (const it of items) {
		const batchId = payoutItemCfg.fieldMap.batchId
			? it?.[payoutItemCfg.fieldMap.batchId]
			: null;
		if (!batchId || !committedBatchIds.has(String(batchId))) continue;
		const status = payoutItemCfg.fieldMap.status
			? it?.[payoutItemCfg.fieldMap.status]
			: null;
		if (status === "cancelled") continue;
		const amount = Number(
			payoutItemCfg.fieldMap.amount ? it?.[payoutItemCfg.fieldMap.amount] : 0,
		);
		if (!Number.isFinite(amount)) continue;
	...
	const availableBalance =
		totalConfirmedRevenue - totalPaidOut - totalCommittedToPayouts;
	return {
...
async function createPayoutBatchesFromEarnings(
	...
	if (!dryRun) {
		if (!shouldWritePayoutLedger())
			throw new Error(
				"Refusing create payout batches without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
			);
	...
	if (settlementId && earningCfg.fieldMap.settlementId) {
		...
		earnings = await listAll(earningEntity, {
	...
	if (beneficiary && earningCfg.fieldMap.beneficiary) {
		filtered = filtered.filter(
			(e) =>
				String(e?.[earningCfg.fieldMap.beneficiary] ?? "") ===
				String(beneficiary),
		);
	}
	...
	for (const e of filtered) {
		const b = earningCfg.fieldMap.beneficiary
			? e?.[earningCfg.fieldMap.beneficiary]
			: null;
		...
		if (!explicitTypeRaw && shouldOptimizePaymentRouting()) {
			const amount = earningCfg.fieldMap.amount
				? e?.[earningCfg.fieldMap.amount]
				: null;
			const currency = earningCfg.fieldMap.currency
				? e?.[earningCfg.fieldMap.currency]
				: null;
		...
		if (!byRecipient.has(key))
			byRecipient.set(key, {
				beneficiary: String(b ?? ""),
				recipientType: derivedType,
				earnings: [],
			});
	...
	const resolveRevenueEventId = async (earning) => {
		const meta = earningCfg.fieldMap.metadata
			? earning?.[earningCfg.fieldMap.metadata]
			: null;
	...
	for (const {
		beneficiary: benefKey,
		recipientType: recipType,
		earnings: group,
	} of byRecipient.values()) {
		const currencies = new Map();
		for (const e of group) {
			const c = earningCfg.fieldMap.currency
				? e?.[earningCfg.fieldMap.currency]
				: null;
			const k = String(c ?? "");
		...
		for (const [currencyKey, list] of currencies.entries()) {
			const earningIds = list
				.map((e) =>
					earningCfg.fieldMap.earningId
						? e?.[earningCfg.fieldMap.earningId]
						: null,
				)
				.filter(Boolean)
				.map((v) => String(v));
			...
			for (const e of list) {
				...
				const eid = earningCfg.fieldMap.earningId
					? e?.[earningCfg.fieldMap.earningId]
					: null;
				missingRecipientEarningIds.push(
					eid ? String(eid) : String(e?.id ?? ""),
				);
			}
			...
			if (allowedPolicy.configured === true) {
				for (const e of list) {
					...
					if (recipientKind === "bank_wire") {
						if (
							!allowedPolicy.bankWireAccounts ||
							allowedPolicy.bankWireAccounts.size === 0
						)
							continue;
						const d = resolveBankWireDestinationFromMeta(meta);
						if (d && isBankWireDestinationAllowedByPolicy(d, allowedPolicy))
							continue;
					} else {
						const recipient = resolveRecipientAddress(
							recipType,
						...
						if (isRecipientAllowedByPolicy(recipType, recipient, allowedPolicy))
							continue;
					}
					const eid = earningCfg.fieldMap.earningId
						? e?.[earningCfg.fieldMap.earningId]
						: null;
					notAllowedRecipientEarningIds.push(
						eid ? String(eid) : String(e?.id ?? ""),
					);
				}
			}
			...
			const existingBatch = await findOneBy(payoutBatchEntity, {
					...
					: await payoutBatchEntity.create({
							...
							[payoutBatchCfg.fieldMap.totalAmount]: Number(
								list
									.reduce(
										(sum, e) =>
											sum + Number(e?.[earningCfg.fieldMap.amount] ?? 0),
										0,
									)
									.toFixed(2),
							),
							...
							...(payoutBatchCfg.fieldMap.settlementId && settlementId != null
								...
								: {}),
							...(payoutBatchCfg.fieldMap.earningIds
								? { [payoutBatchCfg.fieldMap.earningIds]: earningIds }
								: {}),
										...
										[payoutBatchCfg.fieldMap.notes]: {
											...
											recipient_email:
												resolveRecipientAddress(
													recipType,
													list[0]?.[earningCfg.fieldMap.metadata] ?? null,
													benefKey || null,
												) || null,
											payer_email:
			...
			if (
				...
				await payoutBatchEntity.update(batchInternalId, {
					[payoutBatchCfg.fieldMap.revenueEventIds]: revenueEventIds,
				});
			}

			const itemsCreated = [];
			for (const e of list) {
				const earningId = earningCfg.fieldMap.earningId
					? e?.[earningCfg.fieldMap.earningId]
					: null;
				if (!earningId) continue;
				const itemId = makeStableItemId({
					earningId: String(earningId),
					batchId,
					day,
				});

				const existingItem = await findOneBy(payoutItemEntity, {
				...
				const amount = Number(
					earningCfg.fieldMap.amount ? e?.[earningCfg.fieldMap.amount] : 0,
				);
				const cur = earningCfg.fieldMap.currency
					? e?.[earningCfg.fieldMap.currency]
					: null;
				const rid = await resolveRevenueEventId(e);
				const meta = earningCfg.fieldMap.metadata
				...
				const createdItem = await payoutItemEntity.create({
					...
					...(payoutItemCfg.fieldMap.earningId
						? { [payoutItemCfg.fieldMap.earningId]: String(earningId) }
						: {}),
					...(payoutItemCfg.fieldMap.revenueEventId && rid
						? { [payoutItemCfg.fieldMap.revenueEventId]: rid }
						: {}),
				});
...
async function getPayoutItemsForBatch(base44, batchId) {
	...
	const items = await filterAll(
		itemEntity,
		{ [payoutItemCfg.fieldMap.batchId]: String(batchId) },
		{ fields, pageSize: 500 },
	);
...
async function reportApprovedBatches(base44) {
	...
	try {
		...
		batches = batches.filter(
			(b) => b?.[payoutBatchCfg.fieldMap.status] === "approved",
		);
	}
	return batches;
}
...
async function submitPayPalPayoutBatch(
	...
	const rec = await findOneBy(batchEntity, {
		[payoutBatchCfg.fieldMap.batchId]: String(batchId),
	});
	if (!rec?.id) throw new Error(`PayoutBatch not found: ${batchId}`);

	const status = payoutBatchCfg.fieldMap.status
		? rec?.[payoutBatchCfg.fieldMap.status]
		: null;
	if (status !== "approved")
		throw new Error(
			`PayoutBatch not approved (status=${String(status ?? "")})`,
		);

	const batchCurrency = payoutBatchCfg.fieldMap.currency
		? rec?.[payoutBatchCfg.fieldMap.currency]
		: null;
	const items = await getPayoutItemsForBatch(base44, batchId);
	if (items.length === 0)
		throw new Error(`No PayoutItems found for batch: ${batchId}`);

	const notes = payoutBatchCfg.fieldMap.notes
		? rec?.[payoutBatchCfg.fieldMap.notes]
		: null;
	const recipientType = notes?.recipient_type ?? notes?.recipientType ?? null;
	if (normalizeRecipientType(recipientType, "paypal") !== "paypal") {
		throw new Error(
			`Refusing PayPal submit for non-paypal batch (recipient_type=${String(recipientType ?? "")})`,
		);
	}

	const mapped = [];
	for (const it of items) {
		const senderItemId = payoutItemCfg.fieldMap.itemId
			? it?.[payoutItemCfg.fieldMap.itemId]
			: null;
		const recipient = payoutItemCfg.fieldMap.recipient
			? it?.[payoutItemCfg.fieldMap.recipient]
			: null;
		const amount = Number(
			payoutItemCfg.fieldMap.amount ? it?.[payoutItemCfg.fieldMap.amount] : 0,
		);
		const currency = String(
			(payoutItemCfg.fieldMap.currency
				? it?.[payoutItemCfg.fieldMap.currency]
				: null) ??
				batchCurrency ??
				"",
		);
		...
		if (!recipient)
			throw new Error(`Missing recipient for item ${String(senderItemId)}`);
		if (!Number.isFinite(amount) || amount <= 0)
			throw new Error(`Invalid amount for item ${String(senderItemId)}`);
		if (!currency)
			throw new Error(`Missing currency for item ${String(senderItemId)}`);
		mapped.push({
	...
	if (mapped.length === 0)
		throw new Error(`No valid payout items for batch: ${batchId}`);
	...
	if (!shouldWritePayoutLedger())
		throw new Error(
			"Refusing submit without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
		);
	requireLiveMode("submit PayPal payout batch");
	verifyNoSandboxPayPal();
	if (!isPayPalPayoutSendEnabled()) {
		throw new Error(
			"PayPal payouts not enabled (set PAYPAL_PPP2_APPROVED=true and PAYPAL_PPP2_ENABLE_SEND=true)",
		);
	}
	...
	for (const it of mapped) {
		if (isRecipientAllowedByPolicy("paypal", it.recipient, allowedPolicy))
			continue;
		throw new Error(
			`Refusing PayPal send to non-owner recipient (${mask(it.recipient, { keepStart: 1, keepEnd: 0 })})`,
		);
	}
	...
	if (!paypalBatchId) {
		throw new Error(
			"PayPal create payout response missing batch_header.payout_batch_id",
		);
	}
	const submittedAt = new Date().toISOString();

	const existingNotes = payoutBatchCfg.fieldMap.notes
		? (rec?.[payoutBatchCfg.fieldMap.notes] ?? {})
		: null;
	const mergedNotes =
		existingNotes &&
		typeof existingNotes === "object" &&
		!Array.isArray(existingNotes)
			? {
					...
					paypal_batch_status:
						...
						existingNotes?.paypal_batch_status ??
						null,
					paypal_time_created:
						response?.batch_header?.time_created ??
						existingNotes?.paypal_time_created ??
						null,
				}
			: {
					paypal_payout_batch_id: String(paypalBatchId),
	...
	await batchEntity.update(rec.id, {
		...(payoutBatchCfg.fieldMap.status
			? { [payoutBatchCfg.fieldMap.status]: "submitted_to_paypal" }
			: {}),
		...(payoutBatchCfg.fieldMap.submittedAt
			? { [payoutBatchCfg.fieldMap.submittedAt]: submittedAt }
			: {}),
		...(payoutBatchCfg.fieldMap.notes
			? { [payoutBatchCfg.fieldMap.notes]: mergedNotes }
			: {}),
	});

	const bySenderId = new Map();
	for (const it of items) {
		const sid = payoutItemCfg.fieldMap.itemId
			? it?.[payoutItemCfg.fieldMap.itemId]
			: null;
	...
	for (const ri of respItems) {
		const payoutItemId =
			ri?.payout_item_id ?? ri?.payout_item?.payout_item_id ?? null;
		const senderItemId =
			ri?.payout_item?.sender_item_id ?? ri?.sender_item_id ?? null;
		const transactionStatus =
			ri?.transaction_status ?? ri?.payout_item?.transaction_status ?? null;
		const internalId = senderItemId
			? bySenderId.get(String(senderItemId))
			: null;
		if (!internalId) continue;
		await itemEntity
			.update(internalId, {
				...(payoutItemCfg.fieldMap.status
					? { [payoutItemCfg.fieldMap.status]: "processing" }
					: {}),
				...(payoutItemCfg.fieldMap.paypalStatus && transactionStatus
					? { [payoutItemCfg.fieldMap.paypalStatus]: String(transactionStatus) }
					: {}),
				...(payoutItemCfg.fieldMap.paypalItemId && payoutItemId
					? { [payoutItemCfg.fieldMap.paypalItemId]: String(payoutItemId) }
					: {}),
			})
			.catch(() => null);
	}
...
async function ensureProofOfLifePayout(
	base44,
	{ recipientType, recipient, amount, currency, batchId, dryRun },
) {
	const payoutBatchCfg = getPayoutBatchConfigFromEnv();
	const payoutItemCfg = getPayoutItemConfigFromEnv();
	const batchEntity = base44.asServiceRole.entities[payoutBatchCfg.entityName];
	...
	const proofRecipientType = normalizeRecipientType(
		recipientType ?? "paypal",
		"paypal",
	);
	const proofRecipient = String(recipient ?? "").trim();
	if (!proofRecipient)
		throw new Error("Missing recipient for proof-of-life payout");
	...
	if (!shouldWritePayoutLedger())
		throw new Error(
			"Refusing proof-of-life payout without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
		);
	requireLiveMode("create proof-of-life payout");

	const existingBatch = await findOneBy(batchEntity, {
		[payoutBatchCfg.fieldMap.batchId]: proofBatchId,
	});
	const batch = existingBatch?.id
		? existingBatch
	...
	const existingItem = await findOneBy(itemEntity, {
...
async function findPayoutBatchByPayPalBatchId(base44, paypalBatchId) {
	...
	for (const b of batches) {
		const notes = payoutBatchCfg.fieldMap.notes
			? b?.[payoutBatchCfg.fieldMap.notes]
			: null;
		const pid =
...
async function syncPayPalBatchToLedger(
	base44,
	{ batchId, paypalBatchId, dryRun },
) {
	const payoutBatchCfg = getPayoutBatchConfigFromEnv();
	const payoutItemCfg = getPayoutItemConfigFromEnv();
	const revenueCfg = getRevenueConfigFromEnv();
	const relaxEvidence = shouldRelaxEvidence();
	...
	const notes = payoutBatchCfg.fieldMap.notes
		? batch?.[payoutBatchCfg.fieldMap.notes]
		: null;
	const recipientType = notes?.recipient_type ?? notes?.recipientType ?? null;
	if (normalizeRecipientType(recipientType, "paypal") !== "paypal") {
		throw new Error(
			`Refusing PayPal sync for non-paypal batch (recipient_type=${String(recipientType ?? "")})`,
		);
	}
	...
	if (!paypalId) {
		return {
			ok: true,
			synced: false,
			reason: "missing_paypal_payout_batch_id",
			batchId: batch?.[payoutBatchCfg.fieldMap.batchId] ?? null,
		};
	}
	...
	const items = await getPayoutItemsForBatch(
		base44,
		batch?.[payoutBatchCfg.fieldMap.batchId] ?? batchId,
	);
	...
	for (const ri of respItems) {
		...
		const transactionStatus =
			ri?.transaction_status ?? ri?.payout_item?.transaction_status ?? null;
		const transactionId =
			ri?.transaction_id ?? ri?.payout_item?.transaction_id ?? null;
		const timeProcessed =
			ri?.time_processed ??
			ri?.time_completed ??
			details?.batch_header?.time_completed ??
			null;
		...
		const ledgerStatus =
			mapPayPalTransactionStatusToLedgerStatus(transactionStatus);

		const patch = {
			...(payoutItemCfg.fieldMap.status && ledgerStatus
				? { [payoutItemCfg.fieldMap.status]: ledgerStatus }
				: {}),
			...(payoutItemCfg.fieldMap.paypalStatus && transactionStatus
				? { [payoutItemCfg.fieldMap.paypalStatus]: String(transactionStatus) }
				: {}),
			...(payoutItemCfg.fieldMap.paypalTransactionId && transactionId
				? {
						[payoutItemCfg.fieldMap.paypalTransactionId]: String(transactionId),
					}
				: {}),
			...(payoutItemCfg.fieldMap.paypalItemId && payoutItemId
				? { [payoutItemCfg.fieldMap.paypalItemId]: String(payoutItemId) }
				: {}),
			...(payoutItemCfg.fieldMap.processedAt && timeProcessed
				? { [payoutItemCfg.fieldMap.processedAt]: String(timeProcessed) }
				: {}),
		};
		...
		if (
			...
			payoutItemCfg.fieldMap.revenueEventId &&
			revenueCfg.fieldMap.status &&
			(transactionId || relaxEvidence)
		) {
				...
				revenueUpdates.push({
					...
					patch: {
						...
						...(revenueCfg.fieldMap.payoutBatchId
							? {
									[revenueCfg.fieldMap.payoutBatchId]:
										batch?.[payoutBatchCfg.fieldMap.batchId] ?? null,
								}
							: {}),
						...(revenueCfg.fieldMap.verificationProof
							? {
									[revenueCfg.fieldMap.verificationProof]: proof,
								}
							: {}),
					},
				});
	...
	const batchPatch = payoutBatchCfg.fieldMap.notes
		? { [payoutBatchCfg.fieldMap.notes]: mergedNotes }
		: {};

	if (!dryRun) {
		if (!shouldWritePayPalPayoutStatus())
			throw new Error(
				"Refusing PayPal sync without BASE44_ENABLE_PAYPAL_PAYOUT_STATUS_WRITE=true",
			);
		if (!shouldWritePayoutLedger())
			throw new Error(
				"Refusing PayPal sync without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
			);
		...
		if (batchStatus && String(batchStatus).toUpperCase().includes("SUCCESS")) {
			const updatedItems = await getPayoutItemsForBatch(
				base44,
				batch?.[payoutBatchCfg.fieldMap.batchId] ?? batchId,
			);
			const allFinal =
				Array.isArray(updatedItems) && updatedItems.length > 0
					? updatedItems.every((it) => {
							const st = payoutItemCfg.fieldMap.status
								? it?.[payoutItemCfg.fieldMap.status]
								: null;
							return (
								st === "success" ||
			...
			if (allFinal && payoutBatchCfg.fieldMap.status && batch?.id) {
				...
				await batchEntity
					.update(batch.id, {
						[payoutBatchCfg.fieldMap.status]: allSuccess
							? "completed"
							: "failed",
						...(payoutBatchCfg.fieldMap.completedAt
							? { [payoutBatchCfg.fieldMap.completedAt]: completedAt }
							: {}),
					})
					.catch(() => null);
			}
...
function computeTruthSourceForBatch({
	externalProviderId,
	notes,
	anyPayPalSignals,
}) {
	if (!externalProviderId || externalProviderId === "NOT_SUBMITTED")
		return "ledger";
	if (
		notes?.paypal_batch_status != null ||
...
function computeTruthStatusForBatch({
	externalProviderId,
	notes,
	itemStatuses,
}) {
	if (!externalProviderId || externalProviderId === "NOT_SUBMITTED")
		return "SIMULATION / UNEXECUTED";
	const bs = String(notes?.paypal_batch_status ?? "").toUpperCase();
	const anyFailed = itemStatuses.some(
		(s) =>
			s === "failed" ||
			s === "refunded" ||
			s === "unclaimed" ||
	...
	const allSuccess =
		itemStatuses.length > 0 && itemStatuses.every((s) => s === "success");
...
async function exportPayoutTruth(base44, { batchId, limit, onlyReal = false }) {
	...
	if (batchId) {
		batches = batches.filter(
			(b) =>
				String(b?.[payoutBatchCfg.fieldMap.batchId] ?? "") === String(batchId),
		);
	}
	if (limit && Number.isFinite(Number(limit)) && Number(limit) > 0) {
	...
	const rows = [];
	for (const b of batches) {
		const internalBatchId = payoutBatchCfg.fieldMap.batchId
			? b?.[payoutBatchCfg.fieldMap.batchId]
			: null;
		const notes = payoutBatchCfg.fieldMap.notes
			? b?.[payoutBatchCfg.fieldMap.notes]
		...
		const externalProviderId =
			notes?.paypal_payout_batch_id ??
			notes?.paypalPayoutBatchId ??
			"NOT_SUBMITTED";
		if (
			onlyReal &&
			(!externalProviderId || String(externalProviderId) === "NOT_SUBMITTED")
		)
			continue;
		const totalAmount = payoutBatchCfg.fieldMap.totalAmount
			? b?.[payoutBatchCfg.fieldMap.totalAmount]
			: null;
		const currency = payoutBatchCfg.fieldMap.currency
			? b?.[payoutBatchCfg.fieldMap.currency]
			: null;

		const items = internalBatchId
			? await filterAll(
					itemEntity,
					{ [payoutItemCfg.fieldMap.batchId]: String(internalBatchId) },
					{ pageSize: 500 },
				)
			: [];
		...
		const anyPayPalSignals = items.some((it) => {
			const ps = payoutItemCfg.fieldMap.paypalStatus
				? it?.[payoutItemCfg.fieldMap.paypalStatus]
				: null;
			const pt = payoutItemCfg.fieldMap.paypalTransactionId
				? it?.[payoutItemCfg.fieldMap.paypalTransactionId]
				: null;
			return !!ps || !!pt;
		});
		...
		const lastProviderSyncAt = coalesceIsoTimestamp([
			...
			...items.map((it) =>
				payoutItemCfg.fieldMap.processedAt
					? it?.[payoutItemCfg.fieldMap.processedAt]
					: null,
			),
		]);

		const truthSource = computeTruthSourceForBatch({
			externalProviderId,
			notes,
			anyPayPalSignals,
		});
		const truthStatus = computeTruthStatusForBatch({
			externalProviderId,
			notes,
			itemStatuses,
		});
		...
		rows.push({
			...
			providerKind:
				normalizeRecipientType(
					notes?.recipient_type ?? notes?.recipientType ?? null,
					"paypal",
				) === "paypal"
					? "paypal_payouts"
					: null,
			...
			truthSource,
			truthStatus,
			ledgerStatus: payoutBatchCfg.fieldMap.status
				? b?.[payoutBatchCfg.fieldMap.status]
				: null,
		});
	}
...
async function repairSubmittedWithoutProviderId(base44, { limit, dryRun }) {
	...
	for (const b of batches) {
		const ledgerStatus = payoutBatchCfg.fieldMap.status
			? b?.[payoutBatchCfg.fieldMap.status]
			: null;
		if (ledgerStatus !== "submitted_to_paypal") {
			skipped.push({
				batchId: payoutBatchCfg.fieldMap.batchId
					? b?.[payoutBatchCfg.fieldMap.batchId]
					: null,
				reason: "not_submitted_to_paypal",
			});
			continue;
		}
		...
		const patch = {
			...(payoutBatchCfg.fieldMap.status
				...
				: {}),
			...(payoutBatchCfg.fieldMap.submittedAt
				? { [payoutBatchCfg.fieldMap.submittedAt]: null }
				: {}),
			...(payoutBatchCfg.fieldMap.approvedAt
				? {
						[payoutBatchCfg.fieldMap.approvedAt]:
							b?.[payoutBatchCfg.fieldMap.approvedAt] ?? now,
					}
				: {}),
			...(payoutBatchCfg.fieldMap.notes
				? { [payoutBatchCfg.fieldMap.notes]: patchNotes }
				: {}),
		};
		...
		if (!shouldWritePayoutLedger()) {
			throw new Error(
				"Refusing repair without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
			);
		}
...
async function exportPayoneerBatch(base44, { batchId, outPath }) {
	...
	if (normalizeRecipientType(recipientType, "payoneer") !== "payoneer") {
		throw new Error(
			`Refusing Payoneer export for non-payoneer batch (recipient_type=${String(recipientType ?? "")})`,
		);
	}
	...
	const earningEntity = base44.asServiceRole.entities[earningCfg.entityName];
	const token =
		process.env.PAYONEER_PRQ_TOKEN ||
		notes?.prq_token ||
		process.env.PAYONEER_TOKEN ||
		"";
	const prqLink = token
		? `https://link.payoneer.com/Token?t=${String(token)}&src=prqLink`
		: "";
	for (const it of items) {
		const recipient = payoutItemCfg.fieldMap.recipient
			? it?.[payoutItemCfg.fieldMap.recipient]
			: null;
		const amount = payoutItemCfg.fieldMap.amount
			? it?.[payoutItemCfg.fieldMap.amount]
			: null;
		const currency = payoutItemCfg.fieldMap.currency
			? it?.[payoutItemCfg.fieldMap.currency]
			: null;
		const itemId = payoutItemCfg.fieldMap.itemId
			? it?.[payoutItemCfg.fieldMap.itemId]
			: null;
		const recipientEmail =
			recipient && String(recipient).includes("@") ? String(recipient) : "";
		const recipientName = notes?.beneficiary ?? "";
		const note = notes?.note ?? String(batchId);
		const earningId = payoutItemCfg.fieldMap.earningId
			? it?.[payoutItemCfg.fieldMap.earningId]
			: null;
		let payerName =
			notes?.payer_name ?? process.env.SETTLEMENT_REQUESTOR_NAME ?? "";
		let payerEmail =
			notes?.payer_email ?? process.env.SETTLEMENT_REQUESTOR_EMAIL ?? "";
		let payerCompany =
			notes?.payer_company ?? process.env.SETTLEMENT_REQUESTOR_COMPANY ?? "";
		let purpose = notes?.purpose ?? process.env.SETTLEMENT_PURPOSE ?? "";
		let reference =
			notes?.reference ?? process.env.SETTLEMENT_REFERENCE ?? String(batchId);
		if (earningId && earningCfg.fieldMap.earningId) {
			const e = await findOneBy(earningEntity, {
				[earningCfg.fieldMap.earningId]: String(earningId),
			});
			const meta = earningCfg.fieldMap.metadata
				? e?.[earningCfg.fieldMap.metadata]
				: null;
			if (meta && typeof meta === "object") {
				...
				payerCompany =
					payerCompany || meta.payer_company || meta.source_company || "";
	...
	if (isUnsafePath(targetPath))
		throw new Error("LIVE MODE NOT GUARANTEED (unsafe export path)");
	...
	const liveProof = {
		...buildLiveProofBase("export_payoneer_batch"),
		internalPayoutBatchId: String(batchId),
		outPath: absTargetPath,
		bytes,
		sha256: digest,
		evidence_policy: shouldRelaxEvidence() ? "relaxed" : "strict",
	};
	return {
		batchId: String(batchId),
		outPath: absTargetPath,
...
async function exportBankWireBatch(base44, { batchId, outPath, destination }) {
	...
	if (normalizeRecipientType(recipientType, "bank_wire") !== "bank_wire") {
		throw new Error(
			`Refusing bank wire export for non-bank batch (recipient_type=${String(recipientType ?? "")})`,
		);
	}
	...
	if (!destination || !normalizeDestinationObject(destination).account) {
		...
		for (const it of items) {
			...
			if (earning === undefined) {
				earning = await findOneBy(earningEntity, {
					[earningCfg.fieldMap.earningId]: key,
				});
			...
			if (!inferred) inferred = d;
				...
				throw new Error(
					"Multiple bank wire destinations detected across earnings in the same batch",
				);
			}
		}
	}
	...
	if (!account)
		throw new Error("Missing destination account for bank wire export");
	...
	const targetPath = outPath
		? String(outPath)
		: `bank_wire_payout_${String(batchId)}.xls`;
	if (isUnsafePath(targetPath))
		throw new Error("LIVE MODE NOT GUARANTEED (unsafe export path)");
	const absTargetPath = path.resolve(process.cwd(), targetPath);
	fs.mkdirSync(path.dirname(absTargetPath), { recursive: true });
	...
	const digest = sha256FileSync(absTargetPath);
	const liveProof = {
		...buildLiveProofBase("export_bank_wire_batch"),
		internalPayoutBatchId: String(batchId),
		outPath: absTargetPath,
		bytes,
		sha256: digest,
...
async function reportPendingApprovalBatches(base44) {
	...
	try {
		...
		batches = batches.filter(
			(b) => b?.[payoutBatchCfg.fieldMap.status] === "pending_approval",
		);
	}
	return batches;
}
...
async function approvePayoutBatch(base44, { batchId, args, dryRun }) {
	...
	const totalAmount = Number(
		payoutBatchCfg.fieldMap.totalAmount
			? rec?.[payoutBatchCfg.fieldMap.totalAmount]
			: 0,
	);
	if (!Number.isFinite(totalAmount))
		throw new Error("PayoutBatch total_amount is missing/invalid");
	...
	if (Number.isFinite(limit) && limit > 0) {
		...
		let approvedSum = 0;
		for (const b of all) {
			const st = payoutBatchCfg.fieldMap.status
				? b?.[payoutBatchCfg.fieldMap.status]
				: null;
			...
			const at =
				parseDateMs(
					payoutBatchCfg.fieldMap.approvedAt
						? b?.[payoutBatchCfg.fieldMap.approvedAt]
						: null,
				) ??
				parseDateMs(
					payoutBatchCfg.fieldMap.submittedAt
						? b?.[payoutBatchCfg.fieldMap.submittedAt]
						: null,
				) ??
	...
	if (dryRun)
		return { dryRun: true, batchId, approvedAt: new Date().toISOString() };
	if (!shouldWritePayoutLedger())
		throw new Error(
			"Refusing approval without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
		);
...
async function cancelPayoutBatch(base44, { batchId, dryRun }) {
	...
	if (!dryRun) {
		if (!shouldWritePayoutLedger())
			throw new Error(
				"Refusing cancel without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
			);
	...
	if (!dryRun) {
		await batchEntity.update(rec.id, {
			...
			...(payoutBatchCfg.fieldMap.cancelledAt
				? { [payoutBatchCfg.fieldMap.cancelledAt]: cancelledAt }
				: {}),
		});

		for (const it of items) {
			if (!it?.id) continue;
			await itemEntity.update(it.id, {
				[payoutItemCfg.fieldMap.status]: "cancelled",
			});

			const rid = payoutItemCfg.fieldMap.revenueEventId
				? it?.[payoutItemCfg.fieldMap.revenueEventId]
				: null;
			if (rid && revenueCfg.fieldMap.status) {
				const rev = await findOneBy(revenueEntity, { id: rid }).catch(
					() => null,
				);
				if (rev?.id) {
					...
					if (status !== "paid_out") {
						const patch = {};
						if (revenueCfg.fieldMap.payoutBatchId)
							patch[revenueCfg.fieldMap.payoutBatchId] = null;
		...
		if (Number.isFinite(amount)) {
			const createdTx = await txEntity.create({
				...
				[txCfg.fieldMap.description]:
					`Payout batch ${batchId} cancelled, funds returned to available balance`,
				[txCfg.fieldMap.transactionDate]: cancelledAt,
				[txCfg.fieldMap.category]: "other",
				[txCfg.fieldMap.status]: "completed",
				...(txCfg.fieldMap.payoutBatchId
					? { [txCfg.fieldMap.payoutBatchId]: batchId }
					: {}),
			});
	...
	const liveProof = dryRun
		? null
		: {
				...buildLiveProofBase("cancel_payout_batch"),
				internalPayoutBatchId: String(batchId),
				cancelledAt,
				transactionLogId: txId,
			};
	return {
		batchId,
		cancelledAt,
...
async function reportStuckPayouts(
	base44,
	{ batchHours = 24, itemHours = 24 } = {},
) {
	const payoutBatchCfg = getPayoutBatchConfigFromEnv();
	const payoutItemCfg = getPayoutItemConfigFromEnv();
	...
	const batches = await listAll(batchEntity, {
		fields: batchFields,
		pageSize: 250,
	});
	const stuckBatches = [];
	for (const b of batches) {
		...
		stuckBatches.push({
			batchId: payoutBatchCfg.fieldMap.batchId
				...
				: null,
			status,
			totalAmount: payoutBatchCfg.fieldMap.totalAmount
				? b?.[payoutBatchCfg.fieldMap.totalAmount]
				: null,
			currency: payoutBatchCfg.fieldMap.currency
				? b?.[payoutBatchCfg.fieldMap.currency]
				: null,
	...
	const items = await listAll(itemEntity, {
		fields: itemFields,
		pageSize: 250,
	});
	const stuckItems = [];
		...
		const at =
			parseDateMs(
				payoutItemCfg.fieldMap.processedAt
					? it?.[payoutItemCfg.fieldMap.processedAt]
					: null,
			) ?? parseDateMs(it?.created_date ?? null);
		if (at == null || at > itemCutoff) continue;

		stuckItems.push({
			itemId: payoutItemCfg.fieldMap.itemId
				? it?.[payoutItemCfg.fieldMap.itemId]
				: null,
			batchId: payoutItemCfg.fieldMap.batchId
				? it?.[payoutItemCfg.fieldMap.batchId]
				: null,
			status,
			amount: payoutItemCfg.fieldMap.amount
				? it?.[payoutItemCfg.fieldMap.amount]
				: null,
			currency: payoutItemCfg.fieldMap.currency
				? it?.[payoutItemCfg.fieldMap.currency]
				: null,
			ageHours: Number(((now - at) / (60 * 60 * 1000)).toFixed(2)),
		});
...
async function reportTransactionLogs(
	base44,
	{ fromIso = null, toIso = null, type = null, category = null } = {},
) {
	const txCfg = getTransactionLogConfigFromEnv();
	const txEntity = base44.asServiceRole.entities[txCfg.entityName];
	...
	if (type && txCfg.fieldMap.transactionType) {
		logs = logs.filter(
			(l) => String(l?.[txCfg.fieldMap.transactionType] ?? "") === String(type),
		);
	}
	if (category && txCfg.fieldMap.category) {
		logs = logs.filter(
			(l) => String(l?.[txCfg.fieldMap.category] ?? "") === String(category),
		);
	}
...
async function emitFromMissionsCsv(
	base44,
	cfg,
	earningCfg,
	csvPath,
	{ dryRun, limit, createEarnings, beneficiary },
) {
	...
	for await (const line of rl) {
		...
		const occurredAt =
			row.updated_date || row.created_date || new Date().toISOString();
		...
		const created = await createBase44RevenueEventIdempotent(
			base44,
			cfg,
			event,
			{ dryRun },
		...
		if (createEarnings && beneficiary) {
			...
			const earningCreated = await createBase44EarningIdempotent(
				base44,
				earningCfg,
				earning,
				{ dryRun },
...
async function main() {
	...
	const exportSettlement =
		shouldExportSettlement(args) ||
		getEnvBool("npm_config_export_settlement", false) ||
		getEnvBool("NPM_CONFIG_EXPORT_SETTLEMENT", false);

	const offlineStorePath = args["offline-store"] ?? args.offlineStore ?? null;
	if (offlineStorePath)
		process.env.BASE44_OFFLINE_STORE_PATH = String(offlineStorePath);
	...
	if (
		...
		if (!base44)
			throw new Error(
				"Missing Base44 client; set BASE44_APP_ID/BASE44_SERVICE_TOKEN or use --offline",
			);
		const scanLimit = Number(
			args["scan-limit"] ?? args.scanLimit ?? args.limit ?? "200",
		);
	...
	if (
		args["publish-git-changeset"] === true ||
		args.publishGitChangeset === true
	) {
		if (!base44)
			throw new Error(
				"Missing Base44 client; set BASE44_APP_ID/BASE44_SERVICE_TOKEN",
		...
		const repoRoots = rawRoots
			? String(rawRoots)
					...
					.filter(Boolean)
			: [process.cwd()];
		const out = await publishGitChangeSetsToBase44(base44, {
			repoRoots,
			dryRun: !!dryRun,
		});
		process.stdout.write(`${JSON.stringify(out)}\n`);
	...
	if (args["export-payout-truth"] === true || args.exportPayoutTruth === true) {
		...
		const batchId =
			...
			process.env.NPM_CONFIG_BATCH_ID ??
			null;
		const truthOnlyUiEnabled =
			(process.env.BASE44_ENABLE_TRUTH_ONLY_UI ?? "false").toLowerCase() ===
			"true";
		const onlyReal =
			args["only-real"] === true ||
			args.onlyReal === true ||
			...
			truthOnlyUiEnabled;
		const out = await exportPayoutTruth(base44, {
	...
	if (args["repair-payout-truth"] === true || args.repairPayoutTruth === true) {
		...
		const out = await repairSubmittedWithoutProviderId(base44, {
			limit,
			dryRun: !!dryRun,
		});
		process.stdout.write(`${JSON.stringify(out)}\n`);
	...
	if (
		args["proof-of-life-payout"] === true ||
		args.proofOfLifePayout === true
	) {
		const recipient =
			args.recipient ??
		...
		const recipientType =
			...
			"paypal";
		const amountRaw =
			args.amount ?? args["payout-amount"] ?? args.payoutAmount ?? "0.01";
		const currencyRaw =
			args.currency ?? args["payout-currency"] ?? args.payoutCurrency ?? "USD";
		const batchId =
			args["batch-id"] ??
			args.batchId ??
			args.batch ??
		...
		const approved = await approvePayoutBatch(base44, {
			batchId: ensured.batchId,
			args,
			dryRun: false,
		});
		const submitted = await submitPayPalPayoutBatch(base44, {
			batchId: ensured.batchId,
			args,
			dryRun: false,
		});
		process.stdout.write(
			`${JSON.stringify({ ok: true, ensured, approved, submitted })}\n`,
		);
		return;
	}
	...
	if (
		...
		if (!dryRun) {
			if (!shouldWritePayoutLedger())
				throw new Error(
					"Refusing to write payout ledger without BASE44_ENABLE_PAYOUT_LEDGER_WRITE=true",
				);
			requireLiveMode("create payout batches");
		}
		const settlementId =
		...
		const payoutRecipientType =
			...
			null;
		const fromIso =
			args["payout-from"] ?? args["settlement-from"] ?? args.from ?? null;
	...
	if (
		...
		process.stdout.write(
			`${JSON.stringify({ ok: true, count: batches.length, batches })}\n`,
		);
		return;
	}
	...
	if (
		...
		const batchId =
			...
			null;
		if (!batchId)
			throw new Error("Missing --batch-id for approve-payout-batch");
		const out = await approvePayoutBatch(base44, {
			batchId: String(batchId),
			args,
			dryRun: !!dryRun,
		});
	...
	if (
		...
		const out = await submitPayPalPayoutBatch(base44, {
			batchId: String(batchId),
			args,
			dryRun: !!dryRun,
		});
	...
	if (
		...
		if (!batchId && !paypalBatchId)
			throw new Error(
				"Missing --batch-id or --paypal-batch-id for sync-paypal-ledger-batch",
			);
		const out = await syncPayPalBatchToLedger(base44, {
			batchId: batchId != null ? String(batchId) : null,
	...
	if (
		...
		const batchId =
			...
			null;
		if (!batchId)
			throw new Error("Missing --batch-id for export-payoneer-batch");
		const outPath =
			args.out ?? args["out"] ?? args["out-path"] ?? args.outPath ?? null;
		const out = await exportPayoneerBatch(base44, {
			batchId: String(batchId),
			outPath,
		});
		process.stdout.write(`${JSON.stringify({ ok: true, ...out })}\n`);
	...
	if (
		...
		const batchId =
			...
			null;
		if (!batchId)
			throw new Error("Missing --batch-id for export-bank-wire-batch");
		const outPath =
			args.out ?? args["out"] ?? args["out-path"] ?? args.outPath ?? null;
		const destination = {
			...(args.destination
				? parseDestinationJson(String(args.destination))
			...
			...(args["dest-account"]
				? { account: String(args["dest-account"]) }
				: {}),
			...(args["dest-beneficiary"]
				? { beneficiary: String(args["dest-beneficiary"]) }
				: {}),
		};
		const out = await exportBankWireBatch(base44, {
			batchId: String(batchId),
			outPath,
			destination,
		});
	...
	if (
		...
		const out = await cancelPayoutBatch(base44, {
			batchId: String(batchId),
			dryRun: !!dryRun,
		});
		process.stdout.write(`${JSON.stringify({ ok: true, ...out })}\n`);
	...
	if (
		...
		const batchHours = Number(
			args["batch-hours"] ?? args.batchHours ?? args.hours ?? "24",
		);
		const itemHours = Number(
			args["item-hours"] ?? args.itemHours ?? args.hours ?? "24",
		);
	...
	if (
		...
		const logs = await reportTransactionLogs(base44, {
			fromIso,
			toIso,
			type,
			category,
		...
		process.stdout.write(
			`${JSON.stringify({ ok: true, count: logs.length, logs })}\n`,
		);
		return;
	}
	...
	const settlement = exportSettlement
		? getSettlementArgs(args, { beneficiary, dryRun })
		: null;

	if (exportSettlement && !beneficiary) {
		throw new Error(
			"export-settlement requires --earning-beneficiary or EARNING_BENEFICIARY",
		);
	}
	...
	if (args.csv) {
		...
		process.stdout.write(
			`${JSON.stringify({ ok: true, ...result, ...(settlementOut ? { settlementOut } : {}) })}\n`,
		);
		return;
	}
	...
	const amountRaw =
		args.amount ??
		getEnvFirst(["npm_config_amount", "NPM_CONFIG_AMOUNT"]) ??
		positional[0] ??
		null;
	const amount = amountRaw ? Number(amountRaw) : null;
	const currencyRaw =
		args.currency ??
		getEnvFirst(["npm_config_currency", "NPM_CONFIG_CURRENCY"]) ??
		positional[1] ??
		cfg.defaultCurrency;
	...
	const externalIdRaw =
		...
		args["external-id"] ??
		getEnvFirst([
			"npm_config_externalid",
			"npm_config_external_id",
			"NPM_CONFIG_EXTERNALID",
			"NPM_CONFIG_EXTERNAL_ID",
		]) ??
		positional[3] ??
		null;
	const externalId = (
		externalIdRaw ? String(externalIdRaw) : `manual_${Date.now()}`
	).toString();
	const occurredAtRaw =
		args.occurredAt ??
		args["occurred-at"] ??
	...
	const sourceRaw =
		args.source ??
		getEnvFirst(["npm_config_source", "NPM_CONFIG_SOURCE"]) ??
		positional[2] ??
		"manual";
	...
	const event = {
		...
		metadata:
			safeJsonParse(
				args.metadata ??
					getEnvFirst(["npm_config_metadata", "NPM_CONFIG_METADATA"]),
				{},
			) ?? {},
	};
	...
	if (createEarnings && beneficiary) {
		...
		const earningRecipientType =
			args["earning-recipient-type"] ??
			args.earningRecipientType ??
			args["recipient-type"] ??
			args.recipientType ??
			null;
		...
		const earning = {
			...
			metadata: {
				...
				...(earningRecipientType
					? { recipient_type: String(earningRecipientType) }
					: {}),
	...
	if (exportSettlement) {
		const out = runSettlementExportCli(settlement);
		process.stdout.write(
			`${JSON.stringify({ ok: true, settlementOut: out })}\n`,
		);
	}
}
...
export {
	normalizeRecipientType,
	resolveRecipientAddress,
	createPayoutBatchesFromEarnings,
