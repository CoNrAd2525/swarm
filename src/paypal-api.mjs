...
export async function getPayPalAccessToken() {
	...
	if (!res.ok) {
		...
		const idLen = String(clientId || "").trim().length;
		const secLen = String(clientSecret || "").trim().length;
		const diag = JSON.stringify({
			...
			base: effectiveBase,
			client_id_len: idLen,
			client_secret_len: secLen,
		});
		throw new Error(
			`PayPal token request failed (${res.status}): ${text} | diag=${diag}`,
		);
	}
...
export async function paypalRequest(
	...
	if (!res.ok) {
		const text = await res.text().catch(() => "");
		throw new Error(
			`PayPal request failed (${res.status}) ${method} ${path}: ${text}`,
		);
	}
