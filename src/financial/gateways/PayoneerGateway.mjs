...
export class PayoneerGateway {
	constructor({ provider = process.env.PAYONEER_PROVIDER, audit } = {}) {
	...
	ensureReady() {
		...
		if (!enabled)
			throw new Error("PayoneerGateway: PAYONEER_ENABLE=true required");
		if (this.provider !== "LIVE") {
			throw new Error(
				"PayoneerGateway: Simulation disabled. Set PAYONEER_PROVIDER=LIVE or disable route",
			);
		}
		...
		if (
			...
			throw new Error(
				"PayoneerGateway: Missing API credentials (PAYONEER_API_BASE/CLIENT_ID/CLIENT_SECRET)",
			);
		}
	}
	...
	normalizeTransactions(transactions) {
		...
		if (!list.length)
			throw new Error("PayoneerGateway: No transactions provided");
		...
		for (const t of list) {
			...
			if (c !== currency)
				throw new Error(
					`PayoneerGateway: Mixed currencies not supported (${c} vs ${currency})`,
				);
		...
		if (!(total > 0))
			throw new Error("PayoneerGateway: Sum of amounts is zero");
		const reference =
			list[0]?.reference ||
			`Payoneer payout ${new Date().toISOString().slice(0, 10)}`;
		return { amount: Number(total.toFixed(2)), currency, reference };
	}

	async executeTransfer(transactions) {
		this.ensureReady();
		const { amount: _amount } = this.normalizeTransactions(transactions);
		...
		throw new Error(
			"PayoneerGateway: LIVE provider integration not implemented. Disable route or integrate Payoneer.",
		);
	}
}
