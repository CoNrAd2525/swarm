...
function getCcxtTimeoutMs() {
	const raw =
		process.env.CCXT_HTTP_TIMEOUT_MS ??
		process.env.BINANCE_HTTP_TIMEOUT_MS ??
		process.env.HTTP_TIMEOUT_MS ??
		"30000";
	const ms = Number(raw);
	if (!ms || Number.isNaN(ms) || ms < 1000) return 30000;
	return Math.min(180000, Math.floor(ms));
}

function redactSensitiveUrlQuery(s) {
	return String(s || "")
...
async function withdrawUSDTBEP20(address, amount) {
	...
	try {
		...
		throw new Error(
			`Binance withdrawal failed: ${redactSensitiveUrlQuery(e?.message || e)}`,
		);
	}
}
...
async function withdrawUSDTBybit(address, amount, network = "ERC20") {
	...
	try {
		...
		const r = await bybit.withdraw("USDT", amount, address, undefined, params);
		return r;
	} catch (e) {
		throw new Error(
			`Bybit withdrawal failed: ${redactSensitiveUrlQuery(e?.message || e)}`,
		);
	}
}
...
export class CryptoGateway {
	...
	async executeTransfer(transactions, { provider = "auto" } = {}) {
		const enabled =
		...
		if (String(provider || "").toLowerCase() === "auto") {
			const envList = parseProviderList(process.env.CRYPTO_PROVIDER_PRIORITY);
			const order =
				envList.length > 0
					? envList
					: ["bitget", "bybit", "mexc", "binance", "binance_cryptobox"];
			let lastError = null;
			let instructionCandidate = null;
			for (const p of order) {
				try {
					const res = await this.executeTransfer(transactions, { provider: p });
					const st = String(res?.status || "").toUpperCase();
					if (st === "INSTRUCTIONS_READY") {
						if (!instructionCandidate) instructionCandidate = res;
						lastError = new Error(`${p}:${st}`);
						continue;
					}
					if (st === "SUBMITTED" || st === "SUBMITTED_WITH_TX") return res;
					if (st === "MISSING_CREDENTIALS" || st === "UNKNOWN_PROVIDER") {
						lastError = new Error(`${p}:${st}`);
						continue;
					}
					if (st === "PREPARED") {
						lastError = new Error(`${p}:PREPARED`);
						continue;
					}
					if (st === "QUEUED") {
						if (!instructionCandidate) instructionCandidate = res;
			...
			throw new Error(
				`crypto_all_providers_failed:${redactSensitiveUrlQuery(
		...
		if (provider === "binance") {
			...
			for (let i = 0; i < 3 && !txId; i++) {
				...
				try {
					...
					if (Array.isArray(hist)) {
						...
						if (applyId) {
							m = hist.find(
								(h) => String(h.id || h.applyId || "") === String(applyId),
							);
						}
						if (!m) {
							...
							m = hist.find(
								(h) =>
									String(h.address || "").toLowerCase() === addrMatch &&
									Math.abs(Number(h.amount) - Number(amount)) < tol,
							);
						}
			...
			if (!txId)
				return { status: "submitted", applyId, provider, network, prepared_at };
			return {
				status: "submitted_with_tx",
				applyId,
				txHash: txId,
				provider,
				network,
		...
		if (provider === "bybit") {
			...
			const nx = transactions[0]?.network || "ERC20";
			if (enabled && hasCreds) {
				try {
					const r = await withdrawUSDTBybit(dest, amount, nx);
					return {
						status: "submitted",
						applyId: r.id,
						provider: "bybit",
				...
				JSON.stringify(
					{
						...
						creds_present: !!(bybitCreds.apiKey && bybitCreds.secret),
						origin: "in_house",
					},
					null,
					2,
		...
		if (provider === "bitget") {
			const hasCreds = !!(
				bitgetCreds.apiKey &&
				bitgetCreds.secret &&
				bitgetCreds.passphrase
			);
			const preferredRaw = String(process.env.BITGET_CHAIN || "bep20");
			const preferredChainApi = mapBitgetChain(preferredRaw);
			const preferredNetworkLabel = (
				preferredChainApi === "BSC" ? "BEP20" : preferredChainApi
			).toUpperCase();
			if (enabled && hasCreds) {
				const tryChains = Array.from(
					new Set([preferredChainApi, "BSC", "ETH", "TRON"]),
				);
					...
					try {
						...
						if (r1 && r1.code === "00000") {
							return {
								...
								network: label,
								prepared_at,
								applyId: r1?.data?.orderId || null,
							};
						}
					} catch {
						// fall through to next chain or manual instructions
					}
				}
			}
			const outDir = "settlements/crypto";
				...
				JSON.stringify(
					{
						...
						action: "withdraw",
						coin: "USDT",
						network: preferredNetworkLabel,
						address: dest,
						amount,
						status: "WAITING_MANUAL_EXECUTION",
						creds_present: hasCreds,
						origin: "in_house",
					},
					null,
					2,
		...
		if (provider === "trust") {
			return {
				status: "QUEUED",
				provider: "trust",
				reason: "NO_PRIVATE_KEYS_ALLOWED",
				network,
				prepared_at,
			};
		}
	...
	async getWithdrawalStatus({
		...
		if (provider === "binance") {
			try {
				const hist = await listWithdrawals(
				...
				const m = hist.find(
					(h) =>
						...
						(amount == null || Number(h.amount) === Number(amount)),
				);
