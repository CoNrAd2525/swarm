...
export class InstructionGateway {
	...
	async create(
		payload,
		{ dir = "settlements/instructions", prefix = "instruction" } = {},
	) {
	...
	async generate(providerId, transactions, title) {
		...
		return this.create(payload, {
			...
			prefix: `instruction_${String(providerId || "platform")}`,
		});
	}
	async check(providerId) {
		...
		const files = fs.existsSync(dir)
			? fs
					.readdirSync(dir)
					.filter((f) => f.includes(`instruction_${String(providerId || "")}`))
			: [];
		return {
			status: files.length > 0 ? "INSTRUCTIONS_READY" : "PENDING",
			provider: providerId,
