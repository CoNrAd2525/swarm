...
export async function loadAutonomousConfig({ configPath }) {
	const resolved = configPath
		? path.resolve(process.cwd(), String(configPath))
		: path.resolve(process.cwd(), "autonomous.txt");
...
export function resolveRuntimeConfig(args, fileCfg) {
	...
	const intervalMs = normalizeIntervalMs(
		args["interval-ms"] ??
			args.intervalMs ??
			process.env.AUTONOMOUS_INTERVAL_MS,
		cfg.intervalMs,
	);
	...
	const payoutSettlementId =
		args["payout-settlement-id"] ??
		process.env.PAYOUT_SETTLEMENT_ID ??
		cfg.payout?.settlementId ??
		null;
	const payoutBeneficiary =
		args["payout-beneficiary"] ??
		process.env.PAYOUT_BENEFICIARY ??
		cfg.payout?.beneficiary ??
		null;
	const payoutRecipientType =
		args["payout-recipient-type"] ??
		process.env.PAYOUT_RECIPIENT_TYPE ??
		cfg.payout?.recipientType ??
		null;
	...
	const payoutDryRun =
		args["payout-live"] === true
			? false
			: cfg.payout?.dryRun !== false &&
				getEnvBool("AUTONOMOUS_PAYOUT_LIVE", false) !== true;

	const minAvailableBalance = normalizeNumber(
		args["min-available-balance"] ??
			process.env.AUTONOMOUS_MIN_AVAILABLE_BALANCE ??
			cfg.payout?.minAvailableBalance ??
			0,
		0,
	);

	const startHourUtc = normalizeHourUtc(
		args["payout-window-start-utc"] ??
	...
	const syncPayPalLimit = normalizeNumber(
		args["sync-paypal-limit"] ??
			process.env.AUTONOMOUS_SYNC_PAYPAL_LIMIT ??
			cfg.payout?.syncPayPalLimit ??
			25,
		25,
	);
	const syncPayPalMinAgeMinutes = normalizeNumber(
		args["sync-paypal-min-age-minutes"] ??
			process.env.AUTONOMOUS_SYNC_PAYPAL_MIN_AGE_MINUTES ??
			cfg.payout?.syncPayPalMinAgeMinutes ??
			10,
		10,
	);
	...
	const pendingAgeMinutes = normalizeNumber(
		args["auto-approve-age-minutes"] ??
			process.env.AUTONOMOUS_AUTO_APPROVE_AGE_MINUTES ??
	...
	const totp =
		(args.totp ?? args["totp"] ?? process.env.AUTONOMOUS_PAYOUT_TOTP ?? null) ||
		null;

	const requirePayPal =
		args["require-paypal"] === true ||
		(args["skip-paypal"] === true
			? false
			: getEnvBool(
					"AUTONOMOUS_REQUIRE_PAYPAL",
	...
	const alertCooldownMs = normalizeIntervalMs(
		process.env.ALERT_COOLDOWN_MS ?? cfg.alerts?.cooldownMs,
		900000,
	);
	...
	const missionHealthMissionId =
		args["mission-id"] ??
		args.missionId ??
		process.env.AUTONOMOUS_MISSION_ID ??
		cfg.missionHealth?.missionId ??
		null;
	const missionHealthLimit = normalizeNumber(
		args["mission-limit"] ??
			args.missionLimit ??
			process.env.AUTONOMOUS_MISSION_LIMIT ??
			cfg.missionHealth?.limit ??
			50,
		50,
	);
	...
	const deadmanEnabled =
		args.deadman === true ||
		getEnvBool("AUTONOMOUS_DEADMAN", cfg.tasks?.deadman !== false);
	const deadmanIntervalMs = normalizeIntervalMs(
		args["deadman-interval-ms"] ??
			process.env.AUTONOMOUS_DEADMAN_INTERVAL_MS ??
			cfg.deadman?.intervalMs,
		300000,
	);
	const deadmanThresholds = (() => {
		const fromEnv = parseJsonMaybe(
			process.env.AUTONOMOUS_DEADMAN_THRESHOLDS_JSON,
		);
		if (fromEnv && typeof fromEnv === "object" && !Array.isArray(fromEnv))
			return fromEnv;
		return cfg.deadman?.thresholds ?? {};
	})();
	const webhookSilenceHours = normalizeNumber(
		deadmanThresholds.webhookSilenceHours ??
	...
	const metricFailureCount = Math.max(
		...
		Math.floor(
			normalizeNumber(
				deadmanThresholds.metricFailureCount ??
	...
	const metricWindowMinutes = Math.max(
		...
		Math.floor(
			normalizeNumber(
				deadmanThresholds.metricWindowMinutes ??
	...
	const backoffMaxMs = normalizeIntervalMs(
		process.env.AUTONOMOUS_BACKOFF_MAX_MS ?? cfg.backoff?.maxMs,
		300000,
	);
	...
	return {
		...
		tasks: {
			...
			autoSubmitPayPalPayoutBatches:
				autoSubmitPayPalPayoutBatchesEnabled === true,
