...
async function executeBitgetWithdrawal() {
	...
	if (!apiKey || !secret || !password) {
		const errorMsg =
			"CRITICAL: Missing Bitget API credentials in .env file. Cannot proceed.";
		log(errorMsg);
	...
	try {
		...
		log(
			`Attempting to withdraw ${amount} ${coin} to ${address} on ${network}...`,
		);
		...
		console.log(
			"✅ Bitget withdrawal initiated successfully. See logs/bitget-settlement.log for details.",
		);
		...
		log(errorMsg);
		console.error(
			"❌ Bitget settlement failed. See logs/bitget-settlement.log for details.",
		);
		try {
			const args = parseArgs(process.argv);
			const coin = String(args.coin || TARGET_WALLET.coin).toUpperCase();
			const network = String(
				args.network || TARGET_WALLET.network,
			).toUpperCase();
			const address = String(args.address || TARGET_WALLET.address);
			const amount = args.amount
				? Number(args.amount)
			...
			const filePath = path.join(
				outDir,
				`bitget_instruction_${Date.now()}.json`,
			);
			const payload = {
				provider: "bitget",
				action: "withdraw",
