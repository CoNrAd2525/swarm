...
async function renderDashboard() {
	...
	const scoutMemory = await readJson(
		path.resolve("data/swarm/scout-memory.json"),
	);
	...
	if (!ledger || !ledger.missions || ledger.missions.length === 0) {
		...
		console.log(
			`   Summary: ${active.length} Active | ${completed.length} Completed | ${failed.length} Failed\n`,
		);
	...
	let missionPlan = await readJson(path.resolve("data/swarm/mission-plan.json"));
	if (!missionPlan) {
		try {
			missionPlan = buildMissionPlan({});
		} catch {
			missionPlan = null;
		}
	}
	console.log("🧭  PLANNED MISSIONS (DEPENDENCIES)");
	console.log(
		"────────────────────────────────────────────────────────────────",
	);
	if (!missionPlan || !Array.isArray(missionPlan.missions)) {
		console.log("   (No mission plan available yet)");
	} else {
		const ready = missionPlan.missions.filter((m) => m.ready);
		const blocked = missionPlan.missions.filter((m) => !m.ready);
		console.log(
			`   Summary: ${missionPlan.summary?.total ?? missionPlan.missions.length} Total | ${ready.length} Ready | ${blocked.length} Blocked\n`,
		);
		const showReady = ready.slice(0, 8);
		if (showReady.length > 0) {
			console.log("   [READY]");
			for (const m of showReady) {
				const st = m.status ? ` (${m.status})` : "";
				console.log(`   ► P${m.phase}: ${m.id}${st} — ${m.title}`);
			}
		} else {
			console.log("   (No ready missions)");
		}
		const showBlocked = blocked.slice(0, 5);
		if (showBlocked.length > 0) {
			console.log("\n   [BLOCKED]");
			for (const m of showBlocked) {
				const miss = Array.isArray(m.missing_dependencies)
					? m.missing_dependencies.join(", ")
					: "";
				console.log(`   ⛔ ${m.id} — missing: ${miss}`);
			}
		}
	}
	console.log("");
	...
	if (daemonState) {
		console.log(
			`   Last Deadman Check: ${new Date(daemonState.lastDeadmanAt).toISOString()}`,
		);
		console.log(`   Consecutive Failures: ${daemonState.consecutiveFailures}`);
		console.log(
			`   Freeze Active: ${daemonState.freeze?.active ? "YES ❄️" : "NO"}`,
		);
		if (daemonState.freeze?.active) {
			const freezeReason =
				daemonState.freeze.reason ??
				daemonState.freeze.violations ??
				daemonState.freeze.violation ??
				null;
			console.log(
				`   ⚠️ FREEZE REASON: ${JSON.stringify(freezeReason)}`,
			);
		}
	} else {
