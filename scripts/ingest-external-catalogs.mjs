...
const coursesCsvPath = csvDir ? path.join(csvDir, "courses.csv") : null;
const testsCsvPath = csvDir ? path.join(csvDir, "practice_tests.csv") : null;

const dist = path.join(process.cwd(), "rank", "output");
...
function buildPayPalLink(itemName, amount) {
	const business = String(process.env.PAYPAL_OWNER_EMAIL || "").trim();
	if (!business) return "/checkout.html#paypal";
...
function writeHtmlPage(filePath, title, items) {
	...
	const html = [
		...
		'<header><div>' +
			title +
			'</div><nav><a href="/index.html">Home</a><a href="/checkout.html">Checkout</a></nav></header>',
		"<main>",
...
function generateProjectsPage() {
	const items = [
		{
			title: "OSHA Prep Tests Pack",
			headline: "Hard-level MCQs, instant readiness",
			price: 29,
		},
...
async function main() {
	...
	const outCourses = path.join(siteData, "external_courses.json");
	const outTests = path.join(siteData, "practice_tests.json");

	if (!coursesCsvPath || !fs.existsSync(coursesCsvPath)) {
		fs.writeFileSync(outCourses, JSON.stringify([], null, 2));
		fs.writeFileSync(outTests, JSON.stringify([], null, 2));
		const report = {
			ok: false,
			reason: "missing_input",
			input: { csv_dir: csvDir, courses_csv: coursesCsvPath, tests_csv: testsCsvPath },
			out: { courses_json: outCourses, tests_json: outTests },
		};
	...
	const testsExists = !!testsCsvPath && fs.existsSync(testsCsvPath);
	const tests = testsExists
		? (await streamCSV(testsCsvPath, 5000)).map((x) => ({
				courseId: x.courseId,
				title: x.title,
	...
	fs.writeFileSync(outCourses, JSON.stringify(courses, null, 2));
	fs.writeFileSync(outTests, JSON.stringify(tests, null, 2));

	writeHtmlPage(
		path.join(dist, "courses_catalog_extra.html"),
		"Additional Courses",
		courses.slice(0, 50).map((c) => ({ title: c.title, headline: c.headline })),
	);

	const prepItems =
		oshaCourses.length || oshaTests.length
			? [
					...oshaCourses.map((c) => ({ title: c.title })),
					...oshaTests.map((t) => ({ title: t.practicetest_title || t.title })),
				]
			: [
					{ title: "OSHA General Industry Prep", headline: "Exam-style questions", price: 29 },
					{ title: "OSHA Construction Safety Prep", headline: "Exam-style questions", price: 29 },
				];
	...
	const report = {
		...
		counts: {
			...
			tests: tests.length,
			osha_courses: oshaCourses.length,
			osha_tests: oshaTests.length,
		},
		out: {
			courses_json: outCourses,
			tests_json: outTests,
			pages: ["courses_catalog_extra.html", "prep_tests.html", "projects.html"],
		},
	};
	process.stdout.write(JSON.stringify(report) + "\n");
}
