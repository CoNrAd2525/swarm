...
const DRY_RUN =
	String(process.env.CRYPTO_DRY_RUN || "").toLowerCase() === "true";
const LIVE_CONFIRM =
...
async function run() {
	...
	if (enable && !DRY_RUN && !OBSERVE_ONLY && !LIVE_CONFIRM) {
		console.error(
			"❌ Live crypto settlement not confirmed. Set CRYPTO_LIVE_CONFIRM=I_CONFIRM_CRYPTO_SETTLEMENT.",
		);
		process.exit(1);
	}
	...
	for (const tx of transactionsToProcess) {
		...
		try {
			...
			if (DRY_RUN) {
				const outDir = path.resolve("settlements/crypto");
				if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
				const filePath = path.join(outDir, `dry_run_withdrawal_${tx.id}.json`);
				const instruction = {
					action: "withdraw",
					amount,
					coin,
				...
				atomicWriteJsonSync(filePath, instruction);
				tx.status = "INSTRUCTIONS_READY";
				tx.details = {
					...(tx.details || {}),
					filePath,
					dry_run: true,
				...
				const receiptPath = path.join(
					RECEIPTS_DIR,
					`crypto_settlement_${tx.id}_dry_run.json`,
				);
				const receipt = {
					timestamp: new Date().toISOString(),
					batch_id: tx.id,
				...
				console.log(`📝 Dry-run instruction generated: ${filePath}`);
				console.log(`🧾 Dry-run receipt recorded: ${receiptPath}`);
				continue;
			}
			console.log(
