  ...
  try {
    ...
    if (fs.existsSync(receiptsDir)) {
      ...
      const recentFiles = files.filter(
        (f) =>
          f.includes("1767528254631") ||
          f.includes("crypto") ||
          f.includes("BATCH_LIVE") ||
          f.includes(batchId),
      );
    ...
    if (fs.existsSync(settlementsDir)) {
      ...
      if (recentFiles.length > 0) {
        ...
        recentFiles.forEach((file) => {
          ...
          console.log(
            `    Transaction Hash: ${parsed.transactionHash || "Not found"}`,
          );
    ...
    console.log(
      "3. Transaction hash will appear in system logs once confirmed",
    );
  } catch (error) {
