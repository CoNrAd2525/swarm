...
function buildPayoneerXls({ items, batchId, payer }) {
	...
	for (const it of items) {
		const recipient =
			process.env.OWNER_PAYONEER_EMAIL || process.env.PAYONEER_EMAIL || "";
		const recipientEmail =
			recipient && String(recipient).includes("@") ? recipient : "";
		...
		const payerEmail =
			payer.email || process.env.SETTLEMENT_REQUESTOR_EMAIL || "";
		const payerCompany =
			payer.company || process.env.SETTLEMENT_REQUESTOR_COMPANY || "";
		const purpose =
			payer.purpose || process.env.SETTLEMENT_PURPOSE || "Service/Settlement";
		const reference =
			payer.reference || process.env.SETTLEMENT_REFERENCE || batchId;
		rows.push([
			recipient,
			recipientEmail,
			recipientName,
...
function inferPayerFromItems(items) {
	const firstMeta =
		items.find((it) => it.metadata && typeof it.metadata === "object")
			?.metadata || {};
	return {
		name:
			firstMeta.payer_name ||
			firstMeta.source_name ||
			firstMeta.client_name ||
			"",
...
async function main() {
	...
	if (!disableBase44) {
		try {
			...
			console.log("⚠️ Base44 unavailable, falling back to archive/owner_bank_requests.csv");
			const csvPath = path.resolve("archive", "owner_bank_requests.csv");
			const rows = readCsv(csvPath);
			items = rows.map((r) => ({
				earning_id: r.reference || `csv_${Date.now()}_${Math.random()}`,
		...
		console.log("ℹ️ Base44 disabled, using archive/owner_bank_requests.csv");
		const csvPath = path.resolve("archive", "owner_bank_requests.csv");
		const rows = readCsv(csvPath);
		items = rows.map((r) => ({
			earning_id: r.reference || `csv_${Date.now()}_${Math.random()}`,
	...
	if (!items.length) {
		console.error("❌ No items found in Base44 or CSV. Aborting (NO SIMULATION).");
		process.exit(1);
	}

	const currency = items[0]?.currency || "USD";
	const beneficiary = items[0]?.beneficiary || "";
	...
	fs.writeFileSync(
		path.join(receiptDir, `payoneer_receipt_${batchId}.json`),
